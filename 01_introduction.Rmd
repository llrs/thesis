...
Disease can be classified based on age at onset as very early, early or adult on-set disease [@satsangi2006c].
...
Anti-TNF$\alpha$ drugs has changed the IBD treatment as it reduced the hospitalization associated with previous treatments, reducing medical costs and risk of surgery as well as induce a better mucosal healing and quality of life for patients [@peyrin-biroulet2011].
...
Sometimes scientist want to understand the relationship between a phenotype they observe and the underlying mechanism.
...
-   Classification of the patients or samples
    ...
    This might be to accurately describe if a patient has one or other related disease of a possible subset of diseases given their condition [ref:needed].
...
-   An overview of the role of each individual block in a biological system
    ...
    An example of this is the search of markers on blood to identify if there are cancer cells on tissues [ref:needed].
...
-   A better understanding of the relationships between the 'omics types
    ...
    For instance, checking that in a particular case or condition is present on a given relationship, and that this relationships follows our model or not [ref:needed].
...
-   Finding a molecular signature.
    ...
    There are several methods that are used to do this [ref:needed].\
    An example of this is when performing eQTL, where a locus is related to the expression change of genes, so finding the right positions that induce a specific expression level is a very common task [ref:needed].
...
-   A predictive model
    ...
    This kind of methods are used improve treatments, diagnosis and prognosis prediction [ref:needed].

-   Impute values

    Some methods aim to accurately guess which values have one block given some other information.
    Missing values can happen for a variety of reasons from practical ones, like a sample not being available, to technical ones, such as laboratory method failing [ref:needed].
...
It is not know how other variables are related between them.
...
In partial correlations some or all of the other variables are considered on how much do they affect the others and this effect is reduced [@yule1907].
...
-   Dimensional Reduction
    ...
    Several tools use this approach: Momix [@cantini_benchmarking_2020], RGCCA [@tenenhaus2017], mixOmics [@rohart2017], STATegRa [@gomez-cabrero2019], ...
