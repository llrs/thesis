...
To diagnose the IBD doctors use endoscopy and/or magnetic resonance imaging and histologies.

Treatments provided for the IBD include, noninflammatory drugs, suppressors and biologics, i.e, anti TNF- anti-IL2, 23, anti-integrin $\alpha4\beta7$.
...
Nevertheless, many patients \[ref:needed\] are refractory or intolerant to those therapies and need to undergo surgery.
...
Several mechanisms have been proposed to drive IBD pathogenesis \[@zhang2014; @hugot2004\]. Some of them are based on a relationship between the immune system and the microbiome \[@silva2016a; @demattos2015\].
It is also unclear if CD and the UC share the same origin considering their different symptoms.

There is also evidence of some genetic component on the onset of the disease, specially if the disease appears very early (less than 2 years old patients) \[@mcgovern2015b; @satsangi2006c\]. Disease can be classified based on age at onset as very early, early or adult on-set disease \[@satsangi2006c\].
...
The role of the gut microbiota has been proposed to play a role in the IBD pathogenesis.
...
In addition, there has been some observations that IBD patients response to treatments that modify their gut microbiota, such as: fecal microbiota transplantation, probiotics and postbiotics \[ref:needed\].
...
Usually the best diagnosis method is to perform a colonoscopy, whether there is inflammation on the gastroinestinal tract on discontinuous regions then is CD.
...
The Montreal classification aims to classify patients according to their age of disease onset, standardized anatomical disease location an disease behaviour.
...
Often patients report feeling better despite lack of muscular healing [@bhattacharya2016a].
...
Patients might progress from an inflammatory phenotype to a stricturing or penetrating one [@satsangi2006a].
...
In addition, the damage usually consists in many neutrophil in the lumen crypt \[@bassolasmolina2018a\].
...
Other measured parameters include, weight, effective weight, fecal calprotectin, C reactive protein (CRP) and hemoglobine.
...
When these drugs are not effective surgery resection might be needed.
...
Corticosteroids can be taken orally, such as prednisolona, prednisona and Budesonide, intravenal, hidrocortisona, metilprednisolona, or via enemas and suppositories.
...
Tiopurines (azatiopurina, mercatiopurina) are immunosuppressants drugs that deactivate key process of limphocytes T that might trigger the inflammation.
...
In the last two decades the IBD treatment has moved from aminosalicylates, corticosteroids and immunomodulators to anti-TNF$\alpha$.
Anti-TNF$\alpha$ drugs has changed the IBD treatment as it reduced the hospitalization associated and risk of surgery as well as induce a better mucosal healing and quality of life for patients [@peyrin-biroulet2011].
...
This builds up also on patients being refractory to the drugs and treatments given [@raine2021].
...
Therefore, predictors of response would result in high benefits for patients and significant savings for public health care systems.
...
If the drugs fail to contain the inflammation and heal the mucosa doctors might recommend to perform a surgery to remove the damaged section.
...
One of the few methods published that shows an interaction between genes and microorganisms on the IBD is to expose the *ex-vivo* sample or cell lines with microbiomes or supernatant of at their culture [@mayorgas2021].
...
Genetics is the next most common data source used to integrate data on the IBD.
Most studies on genetics and IBD are genome-wide association studies [@kumar2019; @knights2013; @huWholeExomeSequencing2021].
No clear mechanism has been proved despite clear and significant findings, probably because IBD relapses and remits with time.
...
Multiple methods and multiple studies have been used to learn more about the interaction between different data and understand the IBD.
...
However, with patients there are not biological replicates like with cell-lines.
...
This step is usually done by the dimension reduction methods... However, sometimes this is not desired or feasible as the correlation does not explain the direction of the causality of the interaction between the variables (if there is any).
...
For the network methods the following output is usually returned: Connections between the variables/nodes, a measure of how strong is the connection (or simply if there is a connection or if there is not one).
