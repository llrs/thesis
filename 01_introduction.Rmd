...
Other words that are used integrati(-on, -ve), multi-omics, pluri-omics.
However, here integration will be used as it is the more general one and not restricted to omics.
...
This in turn has increased the importance of classification, review and comparison of the tools available, as well as, benchmarking these tools against the same dataset [@wu_selective_2019]. Other times only the strategies used are used to classify the methods [@cavill_transcriptomic_2016; @chongComputationalApproachesIntegrative2017].
...
The results and methods of data integration depend on the objective of the method and on the (biological) question.
...
CD is a progressive reincident disease that can affect all the gastrointestinal tract but shows mostly on both terminal ilium and colon with a discontinous inflammation.
...
On CD the disease has a granulome that can appear on any intestinal layer that consists on big multinucleous cells.
while on UC we observe a different damage consisting in many neutrophil in the cript lumen.
...
Sometimes blood analysis are also used to if there are some factors altered such as leucocits, a high globular sedimentation speed, increas of protein C reactive and anemia.
Also if there is an increase of calprotectine in fecal samples is indication of CD disease.
...
There are several articles that point out to a relationship between the immune system and the microbiome.
...
The detection of fecal calprotectin, is a good marker of endocopic activity with sensitivity above 70% and specificity above 80% generally.
...
This inflammation could also present as an ulceration and rectal sparing.
...
With GWAS it has been seen that *NOD2* is one highly relevant for the disease [@momozawa2018].
However, there hasn't been a proposed mechanism for how would this work.
