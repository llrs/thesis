...
An example of this is when performing eQTL analysis, where a locus is related to a change in gene expression [@repnik2016; @hu2021; @jung2020; @dai2019].
...
Missing values can happen for a variety of reasons from practical ones, like a sample not being available, to technical ones, such as laboratory mistake [@yin2019a].
...
Depending on the amount of variables and samples used in studies can be classiffied in two types.
...
Having just the enough samples for the desired statistical power, however, it is not recommended in case some samples cannot be correctly processed.
...
However, when using patients [] replicates can not usually be performed like on vitro experiments.
...
This assumes a linear relationship and it is computationally expensive when there are thousands of variables.
...
Depending on the input and the objective, methods use different mathematical frameworks to process the data.
...
-   Networks
    ...
    Some methods that use these approaches are Reconstructing Integrative Molecular Bayesian Network ([RIMBANET](https://labs.icahn.mssm.edu/zhulab/?s=rimbanet)) [@zhu2012], Bayesian Consensus Clustering ([BCC](https://github.com/ttriche/bayesCC)) [@lock2013] and others like [@virtanen2012].
...
Such methods include Joint and Individual Variation Explained (JIVE) and integrative Non-negative Matrix Factorization ([iNMF](https://github.com/yangzi4/iNMF)) [@yang2016].
