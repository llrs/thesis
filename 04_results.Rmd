...
(ref:poins-boot) Each point represents a bootstrapped sample (colored by model used).
The dispersion is shown by the ellipses.
...
```{r hsct-pca-16s, fig.scap="PCA of 16S data of the HSCT dataset.", fig.cap = "PCA of the 16S data of the HSCT dataset. There are no clear patterns according to the location. (ref:poins-col-shape)"}
...
```{r hsct-pca-RNAseq, fig.scap="PCA of RNAseq data of the HSCT dataset.", fig.cap = "PCA of the RNAseq data of the HSCT dataset. The samples separate according to the location. (ref:poins-col-shape)"}
...
```{r hsct-bootstrap-quality, fig.scap="Characteristics of the samples of the HSCT bootstraped samples.", fig.cap="Dispersion of the bootstraped samples on age and percentage of colon and controls samples. The bootstraps represent a wide range of different composition."}
...
```{r hsct-bootstrap, fig.scap="Bootstrap results in HSCT dataset.", fig.cap="Bootstrap of the models 0, 1.2 and 2.2 in the HSCT dataset. Model 0 and 1.2 have lower inner and outer AVE score, model 2.2 has lower outer score but higher inner value than the bootstrapped. (ref:poins-boot) The point with the black circle is the AVE of the original data."}
...
```{r hsct-features, fig.scap="Upset plot of variables selected in the HSCT dataset.", fig.cap="Upset plot of variables selected in the HSCT dataset. The variables selected on each model from 0 to 2.3 showing the intersection between them regarding genes, panel A, and OTUs, panel B. Genes are common on models of family 2, while OTUs are common on all models."}
...
```{r hasler-pca-16s, fig.scap="PCA of 16S of the Häsler's dataset.", fig.cap = "PCA of 16S of Häsler's dataset. There are some samples that are very different from the others. (ref:poins-col-shape)"}
...
```{r hasler-pca-RNAseq, fig.scap="PCA of RNAseq of the Häsler's dataset.", fig.cap = "PCA of RNAseq of the Häsler's dataset. There are two clear groups according to the location on the first component. (ref:poins-col-shape)"}
...
k <- kable(df, align = rep("c", ncol(df)),
      ...
      caption = "AVE values of RGCCA models in H\u00E4sler's dataset. The inner and the outer AVE scores of multiple models tested on the H\u00E4sler dataset are shown. The model with the hightest inner AVE is model 1.2 followed by model 2.2.")
format_design(k, df)
```
...
```{r hasler-mcia, fig.scap="MCIA dimensions in the Häsler's dataset.", fig.cap="MCIA dimensions in the Häsler's dataset. MCIA first two dimensions of the dataset shows two vertical groups on the first syntethic dimension according to the location of the samples and colored by the state. Each point represents a sample (colored by location and shape by State)."}
...
```{r morgan-pca-16s, fig.scap="PCA of 16S of the Morgan's dataset", fig.cap="PCA of 16S of the Morgan's dataset. There seems to be quite a diverse microbiota based on the first dimension. (ref:poins-col-shape)"}
...
```{r morgan-pca-RNAseq, fig.scap="PCA of RNAseq of the Morgan's dataset", fig.cap="PCA of RNAseq of the Morgan's dataset. There is no clear separation of the two locations on the first dimensions of the PCA. (ref:poins-col-shape)"}
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks in the Morgan's dataset for model 0. 0 indicates no relationship and 1 indicates a strong relationship. Transcriptome and microbiome are linked.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks in the Morgan's dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship. Transcriptome and microbiome are related to the metadata.")
format_design(k, df)
```
...
(ref:morgan-model1-2) Model 1.2 of the Morgan's dataset.
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "(ref:morgan-model1-2) Relationships between the different blocks in the Morgan's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.  Transcriptome is related to the microbiome but weakly and the microbiome is also related to the metadata.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks in the Morgan's dataset for model 2. 0 indicates no relationship and 1 indicates a strong relationship. Transcriptome and the microbiome are linked and related to demographic and location variables.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks in the Morgan's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship. The transcriptome is weakly related to the microbiome and location but much stronger related to the demographic variables.")
format_design(k, df)
```
...
```{r morgan-mcia-plot, fig.scap="MCIA dimensions in the Morgan's dataset.", fig.cap="MCIA dimensions in the Morgan's dataset. MCIA first two dimensions of the dataset shows some separation between samples by location. (ref:poins-col-shape)"}
...
```{r howell-pca-16s, fig.scap="PCA of 16S data of the Howell's dataset", fig.cap = "PCA of 16S data of the Howell's dataset. The first component separates the samples by their location. (ref:poins-col-shape)"}
...
```{r howell-pca-RNAseq, fig.scap="PCA of RNAseq data of the Howell's dataset", fig.cap = "PCA of RNAseq data of the Howell's dataset. There are two groups of samples according to their location. (ref:poins-col-shape)"}
...
```{r howell-mcia, fig.scap="MCIA dimensions in the Howell's dataset.", fig.cap="MCIA dimensions on the Howell's dataset. The first dimensions separates by location. (ref:poins-col-disease)"}
