...
Different methods, centroid, factorial or horst were tested.

<!--# TODO: show plots and numbers -->

The effect of the sparsity value was measured by the number of variables selected and its effect on the AVE scores.

<!--# TODO Show numbers or plot -->

Exploratory analysis with the superblock model was done.
It was used to look for a good model from the data itself.
Also different models without the superblock were explored.
...
The three blocks with the best tau and the centroid scheme were analyzed by changing the weights between 0 and 1 by 0.1 intervals.
...
The best model with these blocks, according to the inner AVE, was that in which the interactions (1) within the host transcriptome, (2) between the host transcriptome
and the localization, (3) between the host transcriptome and(4) the CGH and between the CGH and the other variables were 1, 1, 1/3 and 1/3, respectively.
The first components of each model can be seen in the figure:

<!--# TODO: Add figure 1C of paper -->
...
In either case the family 1 and family 2 models can correctly separate by sample location
(colon or ileum) but not by disease type or inflammation status.
...
The adonis analysis was performed on this dataset to estimate which were the variables that are more relevant.
...
With the permanova analysis we found that 80% and 90% of the variance of normalized RNA-seq data and microbiome data respectively is explained by the variables of location, time point in the treatment, patient id, sex and treatment.
...
Weighted gene co-expression network analysis did not provide relevant links between bacterias and transcriptome as it failed to find an acceptable scale free degree.

<!--# TODO: explain why STATegRa didn't work out -->

When using STATegRa it didn't show
...
The best model of the family 2 confirmed a relationship between the host transcriptome and the location-related variables, while the microbiome was associated with the demographic and location-related variables (see [Fig 3](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0246367#pone-0246367-g003) and [S2](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0246367#pone.0246367.s002) data of [@revilla2021]).
...
Each model had its own dispersion on the same bootstrapped samples

<!--# TODO: Add plot of bootstraps -->
...
This dataset was processed to confirm the resuls on the previous datasets.
