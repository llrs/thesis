...
Both tables (\@ref(tab:hsct-permanova-RNAseq) and \@ref(tab:hsct-permanova-microbiome)) show that location and IBD is a variable that affects the \acr{GE} and microbiome, but there are some interactions that explain similar percentage of variance.
...
format_design(k, df, out = "scale_down")
```
...
With \acr{MCIA} we can observe that the location does not separate the samples (see figure \@ref(fig:morgan-mcia-plot)).
