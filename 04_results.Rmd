...
```{r hasler-model-0}
m0 <- structure(c(0, 1, 1, 0), .Dim = c(2L, 2L), .Dimnames = list(c("RNAseq", 
"micro"), c("RNAseq", "micro")))
k <- kable(as.data.frame(m0), caption.short = "Model 0 of Häsler dataset", 
           caption = "Model 0 of Häsler dataset")
kableExtra::kable_styling(k, latex_options = "hold_position")
```

The first model for family 1 is:

```{r hasler-model-1-1}
m1.1 <- structure(c(0, 0, 1, 0, 0, 1, 1, 1, 0), .Dim = c(3L, 3L), .Dimnames = list(
    c("RNAseq", "micro", "meta"), c("RNAseq", "micro", "meta"
    )))
k <- kable(as.data.frame(m1.1), caption.short = "Model 1.1 of Häsler dataset", 
           caption = "Model 1.1 of Häsler dataset")
kableExtra::kable_styling(k, latex_options = "hold_position")
```

The after optimization of the model of family 1, the best one is:
...
The first model for family 2 is:
...
After optimization of models of family 1 the best model according to the inner AVE score is:
...
We can see here the AVE scores of each of the previous models:
...
In contrast to the HSCT's dataset, the model with the highest inner AVE was model 1.2 but model 2.2 was close to it (see \@ref(tab:hasler-aves).
