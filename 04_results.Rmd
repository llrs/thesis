...
The effect of the sparsity value was measured by its effect on the AVE scores.

![(fig:tau2AVE) Effect of tau on the inner AVE](images/pugets_tau_to_AVE.png)
...
The same data was used to look for a good model from the data itself including a model with a superblock.
...
STATegRa didn't show a value of, the assumption that there is a shared common factor without influence of other categorical variables was too much. In addition, the model is fixed, so it didn't allow to find new or other relationships that are not one to one. 
...
Each model had its own dispersion on the same bootstrapped samples that can be find here:

<!--# TODO: Add plot of bootstraps -->
![](images/hsct-bootstrap-characteristics.png)

This lead to a dispersion on the score of the model.
The lower the dispersion, the more robust the model was to different conditions than in the initial testing.

![](images/hsct-bootstrap.png)

MCIA was applied as a baseline of the integration.

<!--# TODO: Include plot with the result -->
![](images/paper1_MCIA.png)
