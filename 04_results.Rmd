...
```{r intermodel-workflows, fig.scap="inteRmodel functions and workflow.", fig.cap="`inteRmodel` functions and workflow. Functions provided by the inteRmodel package to search and validate models of relationships using RGCCA. Created with BioRender.com"}
...
```{r puget-models, fig.scap="Different RGCCA models on the Puget's dataset.", fig.cap="Different RGCCA models on the Puget's dataset. The different models with the same data showing the sample position on the first components of the CGH  and the transcriptome block. Model 1 and 1.2 with transcriptomics, CGH data and all the data about the samples together. Model 2 and 2.2 with transcriptomics, CGH data and all the data bout the samples on different blocks. Model superblock and superblock.2 have all the data in different blokcs and one block with all the data."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AVE values of RGCCA models on the Puget's dataset.", 
           caption = "AVE values of RGCCA models on the Puget's dataset. Values for both inner and outer AVE of the first canonical component of models 1, 1.2, 2 2.2 and superblock and superblock.2 are shown.")
format_design(k, df)
```
...
```{r hsct-diversity, fig.cap="Microbiome diversity on the HSCT dataset. On the upper section the Shannon effective and on the lower row the Simpson effective diversity splitted by colon and ileum and controls and CD.", fig.scap="Microbiome diversity on the HSCT dataset."}
...
```{r hsct-models, fig.scap="Models on the HSCT dataset.", fig.cap="On the abscissa the transcriptome, on the ordinate the Microbiome. Each square represents a different model of the HSCT dataset. On panel A colored by disease status, on panel B colored by sample location. Model 0 has only transcriptome and microbiome data, models 1 to 1.2 with data about the samples and models 2.1 to 2.3 with data about the samples split in 3 blocks."}
...
k <- kable(df, align = rep("c", ncol(df)), 
      ...
      caption = "Relationships between the different blocks on the HSCT dataset for model 0. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)), 
           ...
           caption = "elationships between the different blocks on the HSCT dataset for model 1.1. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```

When looking for the model that adjust better following this blocks we arrived to model 1.2 thanks to `inteRmodel`, described below on table \@ref(tab:hsct-model1-2):
...
k <- kable(df, align = rep("c", ncol(df)), 
           ...
           caption = "Relationships between the different blocks on the HSCT dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the HSCT dataset for model 2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the HSCT dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)), 
           ...
           caption = "Relationships between the different blocks on the HSCT dataset for model 2.3. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
      caption.short = "The models on the HSCT and their AVE values.",
      caption = "The models on the HSCT and their AVE values. For each model the inner AVE and the outer AVE is presented.")
format_design(k, df)
```
...
```{r hsct-mcia, fig.scap="MCIA dimensions on the HSCT dataset.", fig.cap="MCIA dimensions on the HSCT dataset. MCIA first two synthetic variables. In red circles the colon and in blue triangles the ileum."}
...
```{r hsct-auc-plot, fig.scap="AUC of the RGCCA models on the HSCT dataset.", fig.cap="AUC of the RGCCA models on the HSCT dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the HSCT dataset."}
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "AUC values of RGCCA models on the HSCT dataset.",
...
```{r hsct-features, fig.scap="Upset plot of variables selected on the HSCT dataset.", fig.cap="Upset plot of variables selected on the HSCT dataset. The variables selected on each model from 0 to 2.3 showing the intersection between them regarding genes, panel A, and OTUs, panel B."}
...
df <- data.frame(Model = c("0", "1.1", "1.2", "2.1", "2.2"),
         ...
         check.names = FALSE)
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "AVE values of RGCCA models on H\u00E4sler's dataset.",
      caption = "AVE values of RGCCA models on H\u00E4sler's dataset. The inner and the outer AVE scores of multiple models tested on the H\u00E4sler dataset are shown. The model with the hightest inner AVE is model 1.2.")
format_design(k, df)
```
...
```{r hasler-auc-plot, fig.scap="AUC of RGCCA models on the Häsler's dataset", fig.cap="AUC of models with RGCCA on the Häsler's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Häsler's dataset."}
...
```{r hasler-mcia, fig.scap="MCIA dimensions on the Häsler's dataset.", fig.cap="MCIA dimensions on the Häsler's dataset. MCIA first two dimensions of the dataset colored by state, the shape is according to the location of the samples. Shows two vertical groups on the first syntethic dimension according to the location of the samples."}
...
```{r morgan-pca-16s, fig.scap="PCA of 16S of the Morgan's dataset", fig.cap="PCA of 16S of the Morgan's dataset. The sample are colored by location. "}
include_graphics("images/morgan_pca_16S.png")
```
...
```{r morgan-pca-rnaseq, fig.scap="PCA of RNAseq of the Morgan's dataset", fig.cap="PCA of RNAseq of the Morgan's dataset. The sample are colored by location. "}
include_graphics("images/morgan_pca_RNAseq.png")
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Morgan's dataset for model 0. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Morgan's dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Morgan's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Morgan's dataset for model 2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Morgan's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
```{r morgan-bootstrap, fig.scap="AVE scores of bootstrapped models on Morgans' dataset.", fig.cap="Inner and outer AVE scores of the bootstrapped models 0 1.2 and 2.2. Model 0 does not have sample data. Model 1.2 has microbiome, transcriptonme and sample data in a single block and model 2.2 has microbiome, transcriptome and the sample data split in several blocks."}
...
```{r morgan-auc-plot, fig.scap="AUC of RGCCA models on the Morgan's dataset", fig.cap="AUC of RGCCA models on the Morgan's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Morgan's dataset."}
...
```{r morgan-mcia-plot, fig.scap="MCIA dimensions on the Morgan's dataset.", fig.cap="MCIA dimensions on the Morgan's dataset. MCIA first two dimensions of the dataset. The color and shape match the location of the sample. There is some separation between samples by location."}
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Howell's dataset for model 0. 0 indicates no relationship and 1 indicates a strong relationship.")
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Howell's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks on the Howell's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "AVE values of RGCCA for the Howell's dataset. The inner and the outer AVE scores of multiple models tested on the Howell dataset are shown. The model with the hightest inner AVE is model 1.2.")
format_design(k, df)
```
...
```{r howell-mcia, fig.scap="MCIA dimensions on the Howell's dataset.", fig.cap="MCIA dimensions on the Howell's dataset. The first dimensions separates by location."}
...
```{r howell-auc-plot, fig.scap="AUC of the RGCCA models on the Howell's dataset.", fig.cap="AUC of the RGCCA models on the Howell's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Howell's dataset."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AUC of the RGCCA models on the Howell's dataset",
      caption = "AUC of the RGCCA models on the Howell's dataset. The classification of the location of the sample according to the first componenent of the models shown. Model 0, 2 and 2.2 have a perfect classification of the samples to their respective location.")
format_design(k, df)
```
...
```{r pathways-hsct-howell, fig.scap="Pathways from common genes on HSCT and Howell's dataset.", fig.cap="Significance of pathways on common genes on HSCT and Howell's dataset ordered by p-value, size according to the number of genes on the pathway found on the dataset and color blue for pathways and red for gene ontologies of biological process."}
