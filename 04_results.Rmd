...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short ="AVE values of RGCCA models on Puget's dataset.", 
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "Model 1.2 of HSCT dataset.", 
     ...
     Transcriptome = c(0L, 0L, 0L, 1L, 0L),
       ...
       Demographic = c(0, 0.2, 0, 0, 0.6),
          Location = c(1, 0.1, 0, 0, 0),
              Time = c(0, 0, 0.6, 0, 0),
  check.names = FALSE)
k <- kable(df, caption.short = "Model 2.2", align = rep("c", ncol(df)),
...
```{r hsct-auc-plot, fig.scap="ROC curves of the RGCCA models on the HSCT dataset.", fig.cap="ROC curve of the different datasets with the models from RGCCA on the HSCT dataset."}
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 1.1 of H\u00E4sler dataset", 
...
k <- kable(df, 
           align = rep("c", ncol(df)),
           caption.short = "Model 1.2 of H\u00E4sler dataset", 
...
df <- data.frame(list(
                   ...
                   check.names = FALSE)
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 2.1 of H\u00E4sler dataset", 
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 2.2 of H\u00E4sler dataset", 
...
```{r hasler-auc-plot, fig.scap="AUC for models with RGCCA on the Häsler's dataset", fig.cap="AUC for models with RGCCA on the Häsler's dataset. The AUC was calculated with the first dimension of the gene expression block ability to predict location of the sample."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AUC for models with RGCCA on H\u00E4sler's dataset",
...
k <- kable(df, align = rep("c", ncol(df)),
      caption.short = "The models on the Morgan's dataset and their AVE values.",
      caption = "The models on the Morgan's dataset and their AVE values. For each model the inner and outer AVE value is presented.")
format_design(k, df)
```
...
```{r morgan-auc-plot, fig.scap="AUC plot for the Morgan's dataset", fig.cap="AUC for the Morgan's dataset classificating the localization of the sample according to the first component of the gene expression of the models generated with RGCCA."}
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Models 1.2 of the Howell's dataset.",
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Models 2.2 the of Howell's dataset.",
...
```{r howell-auc-plot, fig.scap="AUC of the RGCCA models on the Howell's dataset.", fig.cap="AUC of the RGCCA models on the Howell's dataset. The classification of the location of the sample according to the first componenent of the models shown."}
