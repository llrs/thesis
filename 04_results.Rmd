...
On both cases it worked well and no batch effect was created when sequencing the samples.
However on the organoids dataset, a change on the matrigel used to produce them introduced a batch effect that made it impossible to compare samples before and after that change (there weren't any shared sample before and after the change of matrigel).
...
`BaseSet` package, built in R, was released for the first time on CRAN on 2020-11-11, nearly two years after the initial work started on [github](https://github.com/llrs/BaseSet).
...
![(#fig:puget-tau2AVE) Effect of tau on the inner AVE. The suggested tau value is the column between the regular grid.](images/pugets_tau_to_AVE.png)

Exploratory analysis with the superblock model was done.
The first two components of the superblock didn't help to explain the biology or classify the tumors:

![(#fig:puget-superblock) First components of the superblock](images/pugets_superblock.png)
...
![(#fig:puget-models) Different models tried with the same data showing the first components of the CGH data and the transcriptome.](images/pugets_models.png)
...
![(#fig:puget-models-bis) Different models tried with the same data showing the first components of the CGH data and the transcriptome.](images/pugets_models.png)
...
<!--# TODO: explain why WGCNA didn't work out: there are some troubles with scales, n not enough for a diversity of samples (or this is for discussion)?-->
...
![(#fig:hsct-models) On the x-axis the transcriptome, on the Y axis the Microbiome. Each square represents a different model of the HSCT dataset. On panel A colored by disease status, on panel B colored by sample location.](images/hsct-models.png)
...
![(#fig:hsct-bootstrap-quality) Dispersion of the bootstraps on the age and percentage of colon and controls samples.](images/hsct-bootstrap-characteristics.png)
...
![(#fig:hsct-bootstrap) The point with the black circle is the AVE of the original data. The dispersion is shown by the ellipses](images/hsct-bootstrap.png)
...
However, due to big amount of small probabilities when using the BaseSet package to calculate which variables are more relevant it couldn't provide a good estimation on time.
...
![(#fig:hsct-mcia) MCIA first two synthetic variables on the IBD related cohorts.](images/paper1_MCIA.png)
...
![(#fig:hsct-auc) ROC curve of the different datasets with the models from RGCCA and the result with MCIA](images/paper1_MCIA.png)
...
![(#fig:hasler-mcia) ROC curve of the different datasets with the models from RGCCA and the result with MCIA](images/paper1_MCIA.png){alt="ROC curve of the different datasets with the models from RGCCA and the result with MCIA"}

MCIA results was high but not as high as the model 2.2.

### Morgan's dataset {#results-morgan}

We tested if results of inteRmodel were consistent on this dataset.
The different models weren't able to separate the samples neither by location or sex.

![(#fig:morgan-models) First component of the transcriptome and microbiome of the Morgan's dataset.](images/morgans_models.png)
...
![(#fig:morgan-bootstrap) Inner and outer AVE scores of the bootstrapped models.](images/morgans_bootstrap.png)
...
![(#fig:howell-models) The three main models on the Howells dataset. Colored by section colon, ileum and shape according to the disease: square, ulcerative colitis; triangle, normal; circle, Crohn's disease.](images/howells_models.png)
...
![(#fig:howell-bootstrap) Bootstrap of the different models.](images/howells_models_bootstrap.png)
