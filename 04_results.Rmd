...
According to the inner \acr{AVE}, the best model was the one in which the weights (1) between the host transcriptome and location, (2) the host transcriptome and the CGH, and (3) the CGH block were linked to variables related to the location with weights of 1, 0.1 and 0.1, respectively.
...
The effect of the superblock and weights on different models to the inner \acr{AVE}.
...
With globaltest the results were similar: sex, \acr{IBD}, location, age and time since diagnosis were able to explain the SESCD score (p-value $5.7 \cdot 10^{-21}$ ).
...
```{r hsct-bootstrap, fig.scap="Bootstrap results on HSCT dataset.", fig.cap="Bootstrap of the models 0, 1.2 and 2.2 on the HSCT dataset. The point with the black circle is the AVE of the original data. The dispersion is shown by the ellipses. Model 0 and 1.2 have lower inner and outer AVE score, model 2.2 has lower outer score but higher inner value than the bootstrapped."}
