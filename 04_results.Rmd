...
<!--# TODO Show taxonomy plots or abundance -->
...
k <- kable(df, align = rep("c", ncol(df)), digits = 2,
           caption.short = "Model 1 for the Puget's dataset.",
           caption = "Model 1 for Puget's dataset. Relationships between the different blocks in the Puget dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship. The matrix represent relations between the gene expression and CGH with the metadata.")
format_design(k, df)
```
...
k <- kable(df, align =  rep("c", ncol(df)), digits = 2,
           caption.short = "Model 1.2 for the Puget's dataset.",
           caption = "Model 1.2 for Puget's dataset. Relationships between the different blocks in the Puget dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship. CGH has a weaker relationship with the metadata than GE. ")
format_design(k, df)
```
...
k <- kable(df, align =  rep("c", ncol(df)), 
           caption.short = "Model 2 for the Puget's dataset.",
           caption = "Model 2 for Puget's dataset. Relationships between the different blocks in the Puget dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "Model 2.2 the for Puget's dataset.",
           caption = "Model 2.2 for Puget's dataset. Relationships between the different blocks in the Puget dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align =  rep("c", ncol(df)),
           ...
           caption = "Relationships between the different blocks in the Puget's dataset for model superblock. 0 indicates no relationship and 1 indicates a strong relationship. The superblock is related to all other blocks but they are only related to the superblock.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "Model with superblock.2 for the Puget's dataset.",
           caption = "Relationships between the different blocks in the Puget's dataset for model superblock.2. 0 indicates no relationship and 1 indicates a strong relationship. In this optimized design the GE and the metadata is related to GE and CGH too. The relationship between GE and CGH is weaker than other relations.")
format_design(k, df)
```
...
```{r puget-superblock, fig.cap="First components of the model superblock.2 which has all the data of the samples on the Puget's dataset. Samples colored and shaped acording to the localization of the tumor. There is a mix of samples of all locations and then some far from it.", fig.scap="Superblock components in Puget's dataset"}
...
<!--# TODO double check interaction RNA-RNA as yes -->

```{r puget-weights, fig.scap="Effect of superblock and weights on the inner AVE on Puget's dataset.", fig.cap="Effect of superblock and weights on the inner AVE on Puget's dataset. Designs with the superblock showed higher inner AVE scores than without it. Interaction yes/no indicates RNA and RNA interaction. Higher inner AVE values are associated with relationships within the GE (p-values shown on top of the violin plots)."}
knitr::include_graphics("images/pugets-weights.png")
```


The different models resulted on the following \acr{AVE} values as reported on \@ref(tab:puget-models-ave):
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AVE values of RGCCA models in the Puget's dataset.", 
           caption = "AVE values of RGCCA models in the Puget's dataset. Values for both inner and outer AVE of the first canonical component of models 1, 1.2, 2 2.2 and superblock and superblock.2 are shown. The model with the highest inner AVE is the model 1.2 and  superblock.2 of those with superblock.")
format_design(k, df)
```
...
df <- data.frame(Factor = c("Location", "IBD", "Sex", "IBD:Sex", "Location:IBD", "Location:Sex", "Location:IBD:Sex", "Residuals"),
                 ...
                 check.names = FALSE)
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "Permanova analysis of transcriptome.",
      caption = "Permanova analysis of transcriptome. The variables and their interactions (shown with \textbf{:}) and the $R^2$ values and the associated p-value. The higher the $R^2$ the more variance is explained by that factor.")
format_design(k, df)
```
...
df <- data.frame(Factor = c("Location", "IBD", "Sex", "IBD:Sex", "Location:IBD", "Location:Sex", "Location:IBD:Sex", "Residuals"),
                 ...
                 check.names = FALSE)
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "Permanova analysis of microbiome",
      caption = "Permanova analysis of microbiome. The variables and their interactions (shown with $\textbf{:}$) and the $R^2$ values and the associated p-value. The higher the $R^2$ the more variance is explained by that factor.")
format_design(k, df)
```
...
```{r hsct-diversity, fig.cap="Microbiome diversity in the HSCT dataset. On the upper section the Shannon effective and on the lower row the Simpson effective diversity splitted by colon and ileum and controls and CD.", fig.scap="Microbiome diversity in the HSCT dataset."}
...
```{r hsct-pca-16s, fig.scap="PCA of 16S data of the HSCT dataset.", fig.cap = "PCA of the 16S data of the HSCT dataset. Samples colored by location of the segment. There are no clear patterns according to the location."}
...
```{r hsct-pca-RNAseq, fig.scap="PCA of RNAseq data of the HSCT dataset.", fig.cap = "PCA of the RNAseq data of the HSCT dataset. Samples colored by location of the segment. The samples separate according to the location."}
...
```{r hsct-wgcna-power, fig.scap="Power evaluation of WGCNA of the HSCT dataset.", fig.cap="Power evaluation of WGCNA of the HSCT dataset. On the ordinate the power on the abscissa on the left the scale free topology model fit; on the right the mean connectivity. There is a low fit even on large power and the mean connectivity is below 100 from the very first value."}
...
```{r hsct-bootstrap-quality, fig.scap="Characteristics of the samples of the HSCT bootstraped samples.", fig.cap="Dispersion of the bootstraped samples on age and percentage of colon and controls samples."}
...
```{r hsct-bootstrap, fig.scap="Bootstrap results in HSCT dataset.", fig.cap="Bootstrap of the models 0, 1.2 and 2.2 in the HSCT dataset. The point with the black circle is the AVE of the original data. The dispersion is shown by the ellipses. Model 0 and 1.2 have lower inner and outer AVE score, model 2.2 has lower outer score but higher inner value than the bootstrapped."}
...
```{r hsct-mcia, fig.scap="MCIA dimensions in the HSCT dataset.", fig.cap="MCIA dimensions in the HSCT dataset. MCIA first two synthetic variables. In red circles the colon and in blue triangles the ileum."}
...
```{r hsct-auc-plot, fig.scap="AUC of the RGCCA models in the HSCT dataset.", fig.cap="AUC of the RGCCA models in the HSCT dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the HSCT dataset."}
...
```{r hsct-features, fig.scap="Upset plot of variables selected in the HSCT dataset.", fig.cap="Upset plot of variables selected in the HSCT dataset. The variables selected on each model from 0 to 2.3 showing the intersection between them regarding genes, panel A, and OTUs, panel B."}
...
```{r hasler-pca-16s, fig.scap="PCA of 16S of the Häsler's dataset.", fig.cap = "PCA of 16S of Häsler's dataset colored by the location of the samples."}
...
```{r hasler-pca-RNAseq, fig.scap="PCA of RNAseq of the Häsler's dataset.", fig.cap = "PCA of RNAseq of the Häsler's dataset. The samples are colored by location sigma and ileum. There are two clear groups according to the location."}
...
k <- kable(m0, align = rep("c", ncol(m0)),
           caption.short = "Model 0 of H\u00E4sler dataset.", 
           caption = "Model 0 of H\u00E4sler dataset. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, m0)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Model 1.1 of the H\u00E4sler dataset. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, 
           ...
           caption = "Model 1.2 of the H\u00E4sler dataset. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Model 2.1 of the H\u00E4sler dataset. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           ...
           caption = "Model 2.2 of the H\u00E4sler dataset. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
df <- data.frame(Model = c("0", "1.1", "1.2", "2.1", "2.2"),
         ...
         check.names = FALSE)
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "AVE values of RGCCA models in H\u00E4sler's dataset.",
      caption = "AVE values of RGCCA models in H\u00E4sler's dataset. The inner and the outer AVE scores of multiple models tested on the H\u00E4sler dataset are shown. The model with the hightest inner AVE is model 1.2.")
format_design(k, df)
```
...
```{r hasler-auc-plot, fig.scap="AUC of RGCCA models in the Häsler's dataset", fig.cap="AUC of models with RGCCA in the Häsler's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Häsler's dataset."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AUC of the RGCCA models in H\u00E4sler's dataset.",
      caption = "AUC of the RGCCA models in H\u00E4sler's dataset. The AUC was calculated with the first dimension of the gene expression block ability to predict location of the sample.")
format_design(k, df)
```
...
```{r hasler-mcia, fig.scap="MCIA dimensions in the Häsler's dataset.", fig.cap="MCIA dimensions in the Häsler's dataset. MCIA first two dimensions of the dataset colored by state, the shape is according to the location of the samples. Shows two vertical groups on the first syntethic dimension according to the location of the samples."}
...
```{r morgan-pca-16s, fig.scap="PCA of 16S of the Morgan's dataset", fig.cap="PCA of 16S of the Morgan's dataset. The sample are colored by location: red circles pouch, blue triangles pre-pouch ileum (PPI). There seems to be quite a diverse microbiota based on the first dimension."}
...
```{r morgan-pca-RNAseq, fig.scap="PCA of RNAseq of the Morgan's dataset", fig.cap="PCA of RNAseq of the Morgan's dataset. The sample are colored by location: red circles pouch, blue triangles prepouch ileum (PPI). There is no clear separation of the two locations on the first dimensions of the PCA."}
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 0 of the Morgan's dataset.",
           caption = "Relationships between the different blocks in the Morgan's dataset for model 0. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 1 of the Morgan's dataset.",
           caption = "Relationships between the different blocks in the Morgan's dataset for model 1. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 1.2 of the Morgan's dataset.",
           caption = "Relationships between the different blocks in the Morgan's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 2 of the Morgan's dataset.",
           caption = "Relationships between the different blocks in the Morgan's dataset for model 2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 2.2 of the Morgan's dataset.",
           caption = "Relationships between the different blocks in the Morgan's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
      caption.short = "AVE values of RGCCA for the Morgan's dataset.",
           caption = "AVE values of RGCCA for the Morgan's dataset. The inner and the outer AVE scores of multiple models tested on the Morgan dataset are shown. The model with the hightest inner AVE is model 1.2.")
format_design(k, df)
```

When exploring the bootstraps of the data we found that model 1.2 is highly variable:

```{r morgan-bootstrap, fig.scap="AVE scores of bootstrapped models on Morgans' dataset.", fig.cap="Inner and outer AVE scores of the bootstrapped models 0, 1.2 and 2.2 on the Morgan's dataset. Model 0 does not have sample data. The ellipses show a normal distribution Model 1.2 has microbiome, transcriptonme and sample data in a single block and model 2.2 has microbiome, transcriptome and the sample data split in several blocks. Model 2.2 shows less variance than all models but lower inner values than model 1.2. "}
include_graphics("images/morgan_bootstrap.png")
```

<!--# TODO Add points of the models on the real dataset -->

In addition the model 2.2 usually has a lower inner \acr{AVE} compared to model 1.2.

```{r morgan-auc-plot, fig.scap="AUC of RGCCA models in the Morgan's dataset", fig.cap="AUC of RGCCA models in the Morgan's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Morgan's dataset."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short="AUC for the Morgan's dataset", 
      caption="AUC for the Morgan's dataset classificating the localization of the sample according to the first component of the gene expression of the models generated with RGCCA.")
format_design(k, df)
```

With \acr{MCIA}:

```{r morgan-mcia-plot, fig.scap="MCIA dimensions in the Morgan's dataset.", fig.cap="MCIA dimensions in the Morgan's dataset. MCIA first two dimensions of the dataset. The color and shape match the location of the sample. There is some separation between samples by location."}
...
The \acr{16S} of this dataset doesn't show any clear pattern regarding the location of the samples according to the firsts dimensions of the \acr{PCA} on \@ref(fig:howell-pca-16s):

```{r howell-pca-16s, fig.scap="PCA of 16S data of the Howell's dataset", fig.cap = "PCA of 16S data of the Howell's dataset. Samples are colored by location. There are no pattern on any of the first do dimensions."}
include_graphics("images/howell_PCA_16S.png")
```

The \acr{PCA} on the transcriptome shows a distinction between colon and ileum \@ref(fig:howell-pca-RNAseq) there are almost two distinct groups according to location:

```{r howell-pca-RNAseq, fig.scap="PCA of RNAseq data of the Howell's dataset", fig.cap = "PCA of RNAseq data of the Howell's dataset. Samples are colored by location. There are two groups of samples according to their location."}
include_graphics("images/howell_PCA_RNAseq.png")
```


This dataset was processed to confirm the results on the previous datasets.
As always first we started with model 0, connecting both the RNAseq and the \acr{16S} blocks:
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 0 of the Howell's dataset.",
           caption = "**Relationships between the different blocks in the Howell's dataset for model 0**. 0 indicates no relationship and 1 indicates a strong relationship.")
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 1.2 of the Howell's dataset.",
           caption = "Relationships between the different blocks in the Howell's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```


Model 1.2, in table \@ref(tab:howell-model1-2), was the best according to the \acr{AVE} score but perform worse when attempting to recreate known biological differences via classifying samples as we can see in \@ref(fig:howell-models).

```{r howell-models, fig.scap="Models from inteRmodel in the Howell's datset.", fig.cap="Models from inteRmodel in the Howell's datset. The three main models, model 0, 1.2 and 2.2 on the Howell's dataset colored by section colon, ileum and shape according to the disease: square, ulcerative colitis; triangle, normal; circle, Crohn's disease. Model 0 has just trancriptomic and microbiome data, model 1.2 has transcriptomic, microbiome and sample data and model 2.2 has transcriptomic, microbiome and sample data split in different blocks."}
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "Model 2.2 of the Howell's dataset.",
           caption = "Relationships between the different blocks in the Howell's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.")
format_design(k, df)
```
...
k <- kable(df, align = rep("c", ncol(df)),
           caption.short = "AVE values of RGCCA for the Howell's dataset.",
           caption = "AVE values of RGCCA for the Howell's dataset. The inner and the outer AVE scores of multiple models tested in the Howell dataset are shown. The model with the hightest inner AVE is model 1.2.")
format_design(k, df)
```
...
```{r howell-mcia, fig.scap="MCIA dimensions in the Howell's dataset.", fig.cap="MCIA dimensions on the Howell's dataset. The first dimensions separates by location."}
...
```{r howell-auc-plot, fig.scap="AUC of the RGCCA models in the Howell's dataset.", fig.cap="AUC of the RGCCA models in the Howell's dataset. The classification of the localization of the sample according to the first component of the gene expression of the models generated with RGCCA on the Howell's dataset."}
...
k <- kable(df, align = rep("c", ncol(df)), 
           caption.short = "AUC of the RGCCA models in the Howell's dataset",
      caption = "AUC of the RGCCA models in the Howell's dataset. The classification of the location of the sample according to the first componenent of the models shown. Model 0, 2 and 2.2 have a perfect classification of the samples to their respective location.")
format_design(k, df)
```
...
There are 3580 selected on model 2.2 in the \acr{HSCT} dataset and 2189 genes on the Howell's dataset.
...
```{r pathways-hsct-howell, fig.scap="Pathways from common genes in HSCT and Howell's dataset.", fig.cap="Significance of pathways on common genes in HSCT and Howell's dataset ordered by p-value, size according to the number of genes on the pathway found on the dataset and color blue for pathways and red for gene ontologies of biological process."}
