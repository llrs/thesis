...
The quality of the initial RNAseq and \acr{16S} data is crucial for a valid integration analysis.
...
In contrast, DNA extraction and \acr{16S} sequencing had to be established and optimized, initially with the guidelines and support from collaborators especially Dr. Ilias Lagkouvardos.
...
We initially processed the \acr{16S} data to obtain \acr{OTUs}
\acr{OTUs} were the standard some years back [@callahan2017].
However, \acr{OTUs} are not comparable between studies even those that use the same primers.
For this reason, and after a suggestion from a reviewer (on the review process of [@revilla2021]), we moved to process later \acr{16S} sequencing datasets to obtain \acr{ASV}.
\acr{ASV} allow the comparison of the taxonomic imputation between studies using the same primers.
However, comparing \acr{16S} taxa \acr{ASV} or \acr{OTUs} is hard and selecting the right tool to compare them is important [@nearing2022].

It is also worth keeping in mind that several species have variable number of \acr{16S} genes.
Higher sequence counts of certain \acr{16S} might not mean higher abundance of that species compared to another with lower \acr{16S} counts.
...
It is however possible to accurately correct for copy number variation on mock populations where the species and the genomes are known (or at least the \acr{16S}), but harder or impossible on samples whose composition is unknown.
...
Furthermore, the precision of \acr{16S} sequencing for classification of taxa is not enough to understand its role in the gut.
Adherent invasive cells, are bacteria share genetic content with but show different behavior [@nadalian2021].
These bacteria are known to be more abundant on patients with \acr{IBD} [@nadalian2021].
...
Other concerns, like primers used, \acr{16S} regions amplified are currently unavoidable.
...
These efforts usually focus on RNAseq, ATAC-seq (DNase-seq), ChIP-seq, small RNAseq, Methyl-seq or proteomics but not \acr{16S}, microbiome or metagenomics data.
...
To my knowledge there is no accurate artificial method to create \acr{16S} datasets or related microbiome datasets.
...
There is also no reference dataset for integration in \acr{IBD} that accurately represents the disease.
There is a large consortium effort with many samples [@humanmicrobiomeprojectconsortium2012], but this does not contain \acr{16S} and RNAseq data from the same location at the same time for as many samples as the datasets used on this thesis.
...
In this thesis there were two in house datasets collected and sequenced: the \acr{HSCT} and the \acr{BARCELONA} dataset.
In addition, we looked up for the most similar datasets published to confirm our results outside our own cohorts.
We found several with intestinal \acr{16S} and RNAseq data from the same location, to compare with our own datasets.

As explained in the methods' section \@ref(methods-hsct), the \acr{HSCT} dataset is a very unique cohort of \acr{CD} patients undergoing hematopoietic stem cell transplant.
...
This treatment is not indicated for \acr{UC}, so this dataset includes only \acr{CD} patients.

The \acr{BARCELONA} dataset (See appendix section \@ref(barcelona)) includes samples from \acr{CD} and \acr{UC} collected prospectively in patients starting biological treatment and followed up for up to a year.
...
Thus, analysis of this cohort could have provided us with a better understanding on the initial relationships between the microbiome and the mucosa of \acr{IBD} patients.

Unfortunately, the \acr{16S} microbial sequence data from the \acr{BARCELONA} cohort was not of enough qualtiy to make reliable analysis and confidently extract hypothesis or relationships (See appendix section \@ref(barcelona)).
...
What happened with the \acr{BARCELONA} dataset highlights the importance of data quality checks.
In the \acr{BARCELONA} cohort despite no batch effect as checked with `experDesign` on the second sequencing, the low diversity indices suggested that there was some problem with the microbiome data.
...
The Puget's dataset provided a good benchmark to test the methods and performance of \acr{RGCCA} works.
\acr{CGH} is not comparable to microbial data, but the microarray data is similar to the RNAseq data from other datasets.
We were not as much interested on the biology as on learning about the \acr{RGCCA} method and its applicability to integrate different omics.

The Häsler \acr{IBD} dataset was obtained using the same sequencing techniques from endoscopic biopsies as our dataset \acr{HSCT} and \acr{BARCELONA}.
The \acr{16S} data was very similar to the \acr{HSCT} dataset.
...
On \acr{HSCT} following our collaborators advise we used IMNGS to annotate its microbiota data [@lagkouvardos2016].
Afterwards we used S\acr{IL}VA database to annotate the microbiome data on their corresponding taxa.

The Morgan pouchitis dataset was also related to \acr{IBD} but it included patients that underwent colectomy and samples are specific of the pouch or pre-pouch ileum.
...
The Howell dataset includes both pediatric \acr{CD} and \acr{UC} (and non-\acr{IBD}) samples.
It is very similar to our \acr{BARCELONA} dataset.
...
Few of these methods have been applied in \acr{IBD} datasets.
However, recently there has been a review focused on integration in \acr{IBD} [@sudhakar2022].
...
In \acr{IBD}, many studies focus on finding some genes or bacteria to answer a narrow question they have in mind, like which bacteria are related to inflammation [@tang2017] or disease activity. 
...
\acr{PERMANOVA} and `globaltest` served that purpose, but they do not give any insight into which specific microbial species are driving the association between the microbiome and the variables [@susin2020].
...
This is in contrast to other publications that relied purely on the Spearman $\rho$ metric [@hasler_uncoupling_2016].
...
We selected \acr{MCIA} as a baseline to compare our method because it works well on a wide range of datasets, has a good documentation as well and it is widespread used.
The method was developed after \acr{RGCCA} and recently there have been publications that show that it outperforms other methods on its versatility on different contexts [@cantini2021].
...
Previous publications using `RGCCA` in \acr{IBD} have focused on validating genes *DUOX2* and *APOA1* as inflammation predictors (@tang2017) from previously published articles [@haberman2014].
Some publication tried to summarize the existing relationships in \acr{IBD} [@hasler_uncoupling_2016], but none were focused on finding the relationships in \acr{IBD} using `RGCCA` as we did.
...
However, we knew that the transcriptome is mainly related to the location of the samples, as we have seen it on the \acr{PCA}s, and we expected that the microbiome would be more related to the patients demographic characteristics and influenced by dietary and other environmental factors.
...
On models of family 2, variables were grouped according to type into the Demographic block, Time block, and Location block (see tables on design of Puget's \@ref(tab:puget-model2-2), \@ref(tab:puget-model2); \acr{HSCT} \@ref(tab:\acr{HSCT}-model2) \@ref(tab:\acr{HSCT}-model2-2), \@ref(tab:\acr{HSCT}-model2-3); Häsler: \@ref(tab:hasler-model-2-1), \@ref(tab:hasler-model-2-2), Morgan's \@ref(tab:morgan-model2), \@ref(tab:morgan-model2-2), Howell's \@ref(tab:howell-model2-2) models).

The exploration of design on Puget's dataset, and the datasets analysis of \acr{HSCT}, Häsler and Morgan's datasets was published after peer review [@revilla2021].

## Evaluating models

To evaluate a model `RGCCA` provides the inner and outer \acr{AVE}; (see section \@ref(rgcca-output)).
As the inner AVE measures how good does the data fit in the model, we used it to evaluate and compare the models.

<!--# TODO Add tables of mean and sd of AVEs on the bootstrapping ? Long enough could be added-->
...
We used the same approach on four different cohorts, with different origins and types of samples, but all related to the \acr{IBD} popultation including \acr{16S} data and intestinal transcriptome.
...
For these reasons gene expression and \acr{16S} blocks were not split into several blocks.

Besides comparing the results of different methods, these models need to be evaluated by the insights they provide on the biological system they are being applied to, in our case \acr{IBD}.
So far the models were only discussed on their technical merits.

It is known that the mucosal transciptome is related to sample localization (i.e. small or large intestine) and disease activity [@criss2021].
This can be seen in the \acr{PCA} which separate colon an ileum on the first dimension: See the \acr{PCA} figures of the different datasets \@ref(fig:hsct-pca-RNAseq), \@ref(fig:hasler-pca-RNAseq) and \@ref(fig:howell-pca-RNAseq).
...
The variability of microbiome difficults finding clear patterns (See the \acr{PCA} figures of the different datasets: \@ref(fig:hsct-pca-16s), \@ref(fig:hasler-pca-16s), \@ref(fig:morgan-pca-16s) and \@ref(fig:howell-pca-16s)).
...
But the role of microbiome in \acr{IBD} has gained a lot of attention in the last 10 years [@khanna2014; @bringiotti2014; @lloyd-price2019].
The microbiome has been associated for instance with treatment response in \acr{CD} and suggested it could be useful for building an improved classifier for \acr{CD} [@douglas2018].

In addition, it is known that microbiome of patients in remission is different from non-\acr{IBD} patients [@halfvarson2017].
So even if two people are apparently healthy at a given time their microbiome might not be similar.

The relapsing nature of \acr{IBD}, suggests that at different time points the gene expression or the microbiome might be different.
...
Even in healthy non-\acr{IBD} patients, multiple samples with microbiota from the same patients could provide common microbiome signature, which can help identify altered microbiome states later on.

Of the cohorts analyzed in this thesis, the \acr{HSCT} and the Häsler dataset are the only ones including time related variables.
...
This leaves the \acr{HSCT} dataset as the only one with multiple timepoints from the same patients.
...
If the same primers are used the \acr{ASV} could be directly compared.
However, the \acr{ASV}' length might be different.
In addition, in this thesis we used dataset obtained with different primers and also \acr{OTUs}
...
So we are are left with comparing \acr{OTUs} (\acr{HSCT} dataset) with \acr{ASV}s (Howell's).
Despite the errors on \acr{ASV} annotation (@edgar2018), \acr{OTUs} from the \acr{HSCT} dataset can be compared to \acr{ASV} from the Howell's dataset.
...
However, on further evaluation via bootstrapping this classification was not significant, as different relatively big amount of \acr{ASV}s were able to classify the samples.
This might be due to the uniqueness of microbial composition to each sample.

In Howell's and Häsler dataset, the best model based on \acr{AVE} is 1.2; which separates the microbiome component by location too (See figure \@ref(fig:hasler-models) and table \@ref(tab:hasler-aves) for Häsler dataset and figures \@ref(fig:howell-models) and table \@ref(tab:howell-models-ave) for Howell dataset).
The 2.2 models according to the inner \acr{AVE} were not that far away from models 1.2.
...
There are already studies on this direction, not only for \acr{IBD} or intestine but for several different human regions [@humanmicrobiomeprojectconsortium2012].

With our studies we hoped that we would provide which bacteria played a role on the disease, or which genes and bacteria are related in \acr{IBD}.
...
In addition the recent recommendation about integration in \acr{IBD} ([@sudhakar2022]) suggests that there is still much to be done to increase the results on these projects and the effectivity.
...
There seems to be a tendency of multi-omics projects to focus on metagenomics, metatranscriptomics and metaproteomics and abandon plain \acr{16S} sequencing [@zhang2019].
This could be explained by the numerous problems that \acr{16S} sequencing has.
Some of these are avoided or solved by using these other omics techniques.
For instance the metatranscriptomics and metaproteomics of bacteria could help detect what is actively being produced by the microbiota and the metagenomics, what genes are actually there not just restricted to \acr{16S} sequences.
But these methods also have their shortcomings.
Metagenomics still does not detect adherent invasive cells and metatranscriptomics and metaproteomics can not explain which bacteria is expressing which genes (even if paired with metagenomics or \acr{16S} data).
...
In summary, as shown with the \acr{PERMANOVA} approach, several factors affect the disease and the relationship between gut's microbiome and gut's mucosa.
...
We hope this provides a first approximation to accurately understand the relationships between genes and bacteria in \acr{IBD}.
