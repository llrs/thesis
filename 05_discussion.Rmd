...
There are reviews that apply different tools to the same datasets [@cantini2021], some are more theoretical [@bersanelli2016], others are focused on a different field [@cavill2016].
...
We briefly considered using `STATegRa`. However, it is not possible to select which interactions between the blocks exists. 
...
This differs to other publications that relied on them to draw their conclusions [@häsler2017].
...
Some publication tried to summarize the existing relationships in IBD [@hasler_uncoupling_2016], but none were focused on finding the relationships on IBD using `RGCCA` as we did.
...
It is known that the mucosal transciptome is related to sample localization (of)i.e. small or large intestine) and disease activity [@criss2021].
...
In healthy humans, it has been suggested that there is a shared microbial patterns among groups of individuals [@arumugam2011]. 
Such theory proposes three groups of similar microbiota composition, named enterotypes. 
However, the enterotypes classification is not unanimously accepted [@koren2013; @cheng2019].
