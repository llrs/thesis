...
PERMANOVA, `adonis` and `globaltest` worked well to identify which variables about the samples were important.
But they do not provide variable selection or give any insight into which specific microbial species are driving the association between the microbiome and the variables [@susin2020].\
...
<!--# TODO Discuss why WGCNA did not work out-->
...
The development of the `experDesign` package helped avoid batch effects due to the `experDesign` package.
...
But these tools have some shortcomings, that are covered by `experDesign`, `OSAT` cannot handle missing values and does not work well for arbitrary batches.
`anticlust` only accepts numerical variables but it is based on a mathematical theorem that makes it very powerful.
`Omixer` has bugs that prevent comparing with the other tools with no possible workaround.
