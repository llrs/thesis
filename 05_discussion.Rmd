...
This treatment is not indicated for \acr{UC}, so this dataset includes only CD patients.

The BARCELONA dataset (See appendix section \@ref(barcelona)) includes samples from CD and \acr{UC} collected prospectively in patients starting biological treatment and followed up for up to a year.
...
The Howell dataset includes both pediatric CD and \acr{UC} (and non-IBD) samples.
...
However, recently there has been a review focused on integration in IBD [@sudhakar2022].
...
[^discussion-1]: See an exploration of this using A\acr{UC}ell <https://bioconductor.org/packages/A\acr{UC}ell> on this website: <https://llrs.github.io/BaseSet_scRNAseq/A\acr{UC}ell.html>.
...
In addition the recent recommendation about integration in IBD ([@sudhakar2022]) suggests that there is still much to be done to increase the results on these projects and the effectivity.
