...
The effect of the sparsity value was measured by its effect on the inner AVE scores and the combination of the different values for each block as can be seen in figure \ref{fig:puget-tau2AVE}.
...
The PCA didn't show any pattern on the microbiome according to the location, as can be seen in the first dimensions of the PCA in figure \ref{fig:hsct-pca-16s}:
...
Following this split, we used \texttt{inteRmodel} (See section above \ref{intermodel}) to find the model that has higher inner AVE for these blocks is the one in table \ref{tab:hsct-model2-2}:
...
MCIA was applied as a baseline of the integration, the first two components were represented similarly to those of the blocks when using RGCCA, see figure \ref{fig:hsct-mcia} of its outcome.
...
The AUC of classifying the transcriptome in colon or ileum segments was compared between the models (see figure \ref{fig:hsct-auc-plot} and with MCIA.
...
If we look at the dataset 16S, the PCA doe not show a pattern as can be seen in figure \ref{fig:hasler-pca-16s}.
...
The PCA on the transcriptome shows a distinction between colon and ileum according to the first dimension of the PCA that can be seen in figure \ref{fig:hasler-pca-rnaseq}.
...
After optimization of models of family 1 the best model according to the inner AVE score is in table \ref{tab:hasler-model-2-2}:
...
This accuracy resulted on high AUC values for all the models, as can be seen in table \ref{tab:hasler-auc}:
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/PCA_16S_morgan} \caption[PCA of 16S of the Morgan's dataset]{PCA of 16S of the Morgan's dataset. The sample are colored by location: red circles pouch, blue triangles pre-pouch ileum (PPI). There seems to be quite a diverse microbiota based on the first dimension.}\label{fig:morgan-pca-16s}
\end{figure}

The PCA on the transcriptome did not show a clear distinction between the prepouch ileum (PPI) and the pouch in figure \ref{fig:morgan-pca-rnaseq} but there is a pattern:

\begin{figure}
\includegraphics[width=1\linewidth]{images/PCA_RNASeq_morgan} \caption[PCA of RNAseq of the Morgan's dataset]{PCA of RNAseq of the Morgan's dataset. The sample are colored by location: red circles pouch, blue triangles prepouch ileum (PPI). There is no clear separation of the two locations on the first dimensions of the PCA.}\label{fig:morgan-pca-rnaseq}
\end{figure}
...
Model 1.2, in table \ref{tab:howell-model1-2}, was the best according to the AVE score but perform worse when attempting to recreate known biological differences via classifying samples as we can see in \ref{fig:howell-models}.
...
The AUC of each model can be seen in table \ref{tab:howell-auc}.
...
This can be seen in the PCAs which separate colon an ileum on the first dimension: See the PCAs of the different datasets \ref{fig:hsct-pca-rnaseq}, \ref{fig:hasler-pca-rnaseq} and \ref{fig:howell-pca-rnaseq}.
...
\includepdf[pages=-, pagecommand={}, templatesize={\textwidth}{\textheight - \voffset}, trim=0 0 0 20pt,]{paper_celest.pdf}
