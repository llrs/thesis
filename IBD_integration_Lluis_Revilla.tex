...
\protect\hyperlink{acronyms_CD}{CD}AI: mean (min-max) & 120 (0-450 \\
\bottomrule
\end{longtable}
...
\hypertarget{ROC}{%
\subsection{\texorpdfstring{ROC- A\protect\hyperlink{acronyms_UC}{UC}}{ROC- AUC}}\label{ROC}}
...
To compare between different models the area under the curve (A\protect\hyperlink{acronyms_UC}{UC}) of the receiver operating characteristic (ROC) was calculated with the \href{https://cran.r-project.org/package=pROC}{pROC} package {[}\protect\hyperlink{ref-robin2011}{194}{]}.
...
According to the inner \protect\hyperlink{acronyms_AVE}{AVE}, the best model was the one in which the weights (1) between the host transcriptome and location, (2) the host transcriptome and the CGH, and (3) the CGH block were linked to variables related to the location with weights of 1, 0.1 and 0.1, respectively.
...
The effect of the superblock and weights on different models to the inner \protect\hyperlink{acronyms_AVE}{AVE}.
...
With the permanova analysis we found that more of the 50\% of the variance of normalized RNAseq data and microbiome data respectively is explained by the variables of location, disease, sex, and the interaction between disease and sex.
...
With globaltest the results were similar: sex, \protect\hyperlink{acronyms_IBD}{IBD}, location, age and time since diagnosis were able to explain the SESCD score (p-value \(5.7 \cdot 10^{-21}\) ).
...
\begin{figure}
\includegraphics[width=1\linewidth]{images/hsct-bootstrap} \caption[Bootstrap results on HSCT dataset.]{Bootstrap of the models 0, 1.2 and 2.2 on the HSCT dataset. The point with the black circle is the AVE of the original data. The dispersion is shown by the ellipses. Model 0 and 1.2 have lower inner and outer AVE score, model 2.2 has lower outer score but higher inner value than the bootstrapped.}\label{fig:hsct-bootstrap}
\end{figure}
...
The \protect\hyperlink{acronyms_AUC}{area under the curve (AUC)} of classifying the transcriptome in colon or ileum segments was compared between the models (see figure \ref{fig:hsct-auc-plot} and with \protect\hyperlink{acronyms_MCIA}{MCIA}).
...
In addition the model 2.2 usually has a lower inner \protect\hyperlink{acronyms_AVE}{AVE} compared to model 1.2.
...
The Häsler \protect\hyperlink{acronyms_IBD}{IBD} dataset was obtained using the same sequencing techniques from endoscopic biopsies as our dataset \protect\hyperlink{acronyms_HSCT}{HSCT} and acr\{BARCELONA\}.
...
Despite all of this, there was large agreement in the genes and pathways identified as relevant between datasets (See figure @ref(fig:pathways-\protect\hyperlink{acronyms_HSCT}{HSCT}-howell)).
