...
The host transcriptome was visually inspected for batch effects in PCA.
Outliers and the top 10% genes using the coefficient of variation were removed (20593, with remaining 37685 genes).
Data was normalized using the trimmed mean of M-values and log transformed into counts per millions using edgeR (version 3.28) .
...
To test if some variables according to a statistic like fold change or value fgsea is a software used to see if groups of variables present a

GSVA is similar but instead of first computing the ordering values and then performing the test it first groups the variables and then they are tested.

adonis method uses to test if microbiome data variation comes from known sources of variation.

microbiome diversity is often measured in two distinct levels
...
WGCNA, co-expression with all the microbiome and RNAseq, not working due to different type of data.

MCIA, simple integration, two blocks

clusterProfiler: genes enrichment for functionality on databases.

globaltest for testing complex hipothesis and help decide if the variables were influencing.

STATegRa, not interaction between variables.
