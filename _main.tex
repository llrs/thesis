...
Differential expression analysis was performed with the limma-trend method {[}\protect\hyperlink{ref-ritchie2015}{175}, \protect\hyperlink{ref-law2014}{176}{]} and edgeR {[}\protect\hyperlink{ref-mccarthy2012}{145}{]} (Bioconductor version 3.10 or superior) packages.
...
The quality of the initial RNAseq and 16S data is important for a valid integration analysis.
...
In contrast, DNA extraction and 16S sequencing had to be established and optimized, initially with the guidelines and support from collaborators {[}mention?{]}
.
After encountering some problems we added blank samples to control for bacterial contamination during sample pre-processing or sequencing.
...
It is however possible to accurately correct for copy number variation on mock populations where the species and the genomes are known (or at least the 16S rRNA), but harder or impossible on samples whose content is unknown.
...
Thus, bacteria with the same complete sequence can play a different role in .

While most of these steps were out of my control as they were performed by a lab teammate or by collaborators.
Other concerns are currently unavoidable or could not have been found beforehand.
They had to consider them as they affect quality and interpretation of the data.
...
Further research on this area might help finding which kind of relationships can be expected given certain dataset sizes.
...
This could allow to benchmark different tools, impute missing data and do pilot studies.
These efforts usually focus on RNA-seq, ATAC-seq (DNase-seq), ChIP-seq, small RNA-seq, Methyl-seq or proteomics but not 16S, microbiome data or metagenomics.
These efforts also focus on relationships between samples missing relationships within cells (regulated DNA, protein recruitment, transcription factors, siRNA regulation,\ldots) and between cells (immune response, adherence, cytokines signaling\ldots) that might surface at the sample level.
...
There is not a reference dataset for integration on IBD that accurately represents the disease.
There is a large consortium effort with many samples{[}\protect\hyperlink{ref-humanmicrobiomeprojectconsortium2012}{4}{]}.
It does not have 16S and RNAseq data from the same location at the same time on as much samples as the datasets used on this thesis.

In order to compare different methods we are limited to comparing different datasets.
...
There is not a resource were datasets of publications are collected with their characteristics.
Finding different datasets for very similarly data is thus challenging.
Furthermore when pooling together datasets batch effect correction will most likely be necessary.
There are tools to overcome this, but currently only work if they share some features or samples in common {[}\protect\hyperlink{ref-ugidos2020}{205}{]}.
These methods usually require datasets with similar RNA sequencing procedures.

On this thesis there were two in house datasets collected and sequenced: the HSCT and the BARCELONA dataset.
For the purpose of this thesis, we looked up for the most similar datasets published in order to be able to confirm our results outside our own cohorts.
We found several with 16S and RNAseq data from the intestine from the same location, to compare with our own datasets.

As explained in methods' section \ref{methods-hsct}, the HSCT dataset is a very unique cohort of CD patients undergoing hematopoietic stem cell transplant.
...
The BARCELONA dataset (See appendix section \ref{barcelona}) includes samples from CD and UC collected propsectively in patients starting biological treatment and followed up for up to a year.
Patients were younger and had less years with the disease.
Patients in HSCTthe group were refactry to treatment and may even be naive to any biologic , which could provide with more insights on the initial relationships between the microbiome and the mucosa of IBD patients.

Unfortunately, the 16S microbiol sequence data of BARCELONA was not good enough to make reliable analysis and confidently extract hypothesis or relationships (See appendix section \ref{barcelona}).
...
Nevertheless, sample sequencing was repeated in an independent sequencing facility to overcome the of the first dataset but problems persisted rendering this data suboptimal for further analysis.

What happened with the BARCELONA dataset highlights the importance of data quality checks.
In the BARCELONA cohort despite no batch effect or other quality issues with the data as checked with \texttt{experDesign}, the diversity indices suggested that there was some problem with the microbiome data.
...
There are reviews of different tools on the same datasets {[}\protect\hyperlink{ref-cantini2021}{129}{]}, some are more theoretical {[}\protect\hyperlink{ref-bersanelli2016}{206}{]}, others are focused on a different field {[}\protect\hyperlink{ref-cavill2016}{99}{]}.
...
We made the assumptions that microbiome presence and transcriptome are related.
...
PERMANOVA and \texttt{globaltest} identified which variables were important.
But they do not give any insight into which specific microbial species are driving the association between the microbiome and the variables {[}\protect\hyperlink{ref-susin2020}{210}{]}.
In addition, we could miss some other important variables.
It is known the role of other factors outside the omics data collected, mainly from environment factors, genetic susceptibility and immune response {[}\protect\hyperlink{ref-sartor2006}{211}{]}.
...
One variable we did not keep track was the microbiome load which is linked to the gut's microbiome community variation {[}\protect\hyperlink{ref-vandeputte2017}{212}{]}.
...
All of these variations didn't provide a clear insight over which genes were correlated to which microbiomes, this differs to other publications that relied on them to draw their conclusions {[}\protect\hyperlink{ref-huxe4sler2017}{214}{]}.
...
We developed \texttt{BaseSet} to find these relationships using a different approach.
...
However, the package was mentioned as \href{https://rviews.rstudio.com/2020/12/22/november-top-40-new-cran-packages/}{top 40 packages} added on CRAN that month and it will be useful on other circumstances or when less combinations are possible.
...
As discussed on the related publication, there are several tools already focused on this problem: \texttt{OSAT} ({[}\protect\hyperlink{ref-yan2012}{216}{]}), \texttt{anticlust} ({[}\protect\hyperlink{ref-papenberg2020}{217}{]}) and \texttt{Omixer} ({[}\protect\hyperlink{ref-sinke2021}{218}{]}).
...
We selected \texttt{MCIA} as a baseline to compare our method because it works well on a wide range of datasets, has a good documentation as well and it is fairly widespread used.
...
Previous publications using \texttt{RGCCA} on IBD focused on validating inflammation markers genes \emph{DUOX2} and \emph{APOA1} as inflammation predictors ({[}\protect\hyperlink{ref-tang2017}{80}{]}) from previously published articles {[}\protect\hyperlink{ref-haberman2014}{219}{]}.
...
To model the relationships the connections between blocks had to be set on \texttt{RGCCA}.
...
It is known that mucosal transciptome is related to localization of the samples {[}\protect\hyperlink{ref-criss2021}{228}{]}.
...
But the role of microbiome on IBD has recently gained some interest {[}\protect\hyperlink{ref-khanna2014}{16}, \protect\hyperlink{ref-bringiotti2014}{233}, \protect\hyperlink{ref-lloyd-price2019}{234}{]}.
It has been associated with predictors of CD treatment response and suggested that they could be useful for building an improved classifier for CD and treatment response based on sufferers' microbiome in the future {[}\protect\hyperlink{ref-douglas2018}{235}{]}.
Thee is for instance, an association between IBD and butyrate producers (See {[}\protect\hyperlink{ref-takahashi2016}{236}{]}) which has shown to influence permeability of the epithelial barrier {[}\protect\hyperlink{ref-ferrer-picuxf3n2020}{24}{]}.

In addition, it is known that microbiome of patients in remission is different from non-IBD patients {[}\protect\hyperlink{ref-halfvarson2017}{237}{]}.
...
Despite the errors on ASV annotation ( {[}\protect\hyperlink{ref-edgar2018}{238}{]}), OTUs from the HSCT dataset can be compared to ASV from Howell's dataset.
...
Some studies using host transcriptomics of fecal wash infer inflammation without colonoscopy {[}\protect\hyperlink{ref-ungar2022}{239}{]}.
...
Krassowski's \emph{et. al.} ({[}\protect\hyperlink{ref-krassowski2020}{94}{]}) advice and recommendations on software engineering and reproducibility practices to share awareness with new researchers in multi-omics for end-to-end workflow.
...
There seems to be a tendency of multi-omics project to focus on metagenomics, metatranscriptomics and metaproteomics and abandon plain 16S sequencing {[}\protect\hyperlink{ref-zhang2019}{240}{]}.
...
Perhaps as suggested in other publications network methods might be able to provide more detailed information about the relationships {[}\protect\hyperlink{ref-jiang2019}{241}{]}.
But, it is unclear how complete and valid are these networks.
There is some evidence that currently literature focus on already known and studied genes instead of more novel and with higher relevance genes {[}\protect\hyperlink{ref-haynes2018}{242}{]}.
...
236. Takahashi K, Nishida A, Fujimoto T, Fujii M, Shioya M, Imaeda H, et al. \href{https://doi.org/10.1159/000441768}{Reduced abundance of butyrate-producing bacteria species in the fecal microbial community in crohn's disease}. Digestion. 2016;93:59--65.

\leavevmode\vadjust pre{\hypertarget{ref-halfvarson2017}{}}%
237. Halfvarson J, Brislawn CJ, Lamendella R, Vázquez-Baeza Y, Walters WA, Bramer LM, et al. \href{https://doi.org/10.1038/nmicrobiol.2017.4}{Dynamics of the human gut microbiome in inflammatory bowel disease}. Nature Microbiology. 2017;2:1--7.

\leavevmode\vadjust pre{\hypertarget{ref-edgar2018}{}}%
238. Edgar R. \href{https://doi.org/10.7717/peerj.5030}{Taxonomy annotation and guide tree errors in 16S rRNA databases}. PeerJ. 2018;6:e5030.
...
242. Haynes WA, Tomczak A, Khatri P. \href{https://doi.org/10.1038/s41598-018-19333-x}{Gene annotation bias impedes biomedical research}. Scientific Reports. 2018;8:1362.
