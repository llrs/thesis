...
\begin{figure}
\includegraphics{images/tesis_AM_Figure2} \caption{The microbial composition in a healthy and IBD gut.}\label{fig:composition}
\end{figure}
...
\begin{figure}
\includegraphics{images/tesis_AC_FigureX} \caption{The intestinal epithelial barrier.}\label{fig:barrier}
\end{figure}
...
\begin{figure}
\centering
\includegraphics{images/workflow_integration2.png}
\caption{\label{fig:workflow-process}Workflow of the analysis process}
\end{figure}
...
To model what might be the relationships within datasets current practices include using a pre-selected model of relations between blocks.
...
To help find the fitting model for the data I created an R package, named \href{https://llrs.github.io/inteRmodel/}{inteRmodel}, which helps finding the right model to the data via a bootstrapping procedure.
...
\[
\begin{aligned}
P & = \frac{ \displaystyle{{a+b}\choose{a}} \displaystyle{{c+d}\choose{c}} }{ \displaystyle{{n}\choose{a+c}} } \\
...
It was published on a journal on 2021-11-27 {[}\protect\hyperlink{ref-revillasancho2021}{193}{]}.
...
On the designed datasets \texttt{experDesign} worked well and no batch effect was created when sequencing the samples.
...
Since its release on CRAN it has had a median of \textasciitilde400 downloads each month from RStudio repository mirror.
...
\item
  Last, some \href{https://docs.ropensci.org/BaseSet/reference/index.html\#utiles}{utility functions} to use set name conventions and some other auxiliary functions.
\end{itemize}

The package had a long development process with initial iterations basing on GSEABase package which was later abandoned (\href{https://github.com/llrs/GSEAdv}{GSEAdv}) to also include some uncertainty on the relationship of a gene with a given gene set.
...
There were three different packages created as part of this process, \texttt{BaseSet}, \texttt{BiocSet} published \href{https://bioconductor.org/packages/BiocSet/}{on Bioconductor} and \texttt{unisets}, available \href{https://github.com/kevinrue/unisets}{on github}.
...
Since its release on CRAN it has had a median of \textasciitilde400 downloads each month from RStudio package manager.

\hypertarget{intermodels}{%
\subsection{inteRmodels}\label{intermodels}}
...
Using the package \href{https://github.com/llrs/intermodel}{on github} simplifies the process and makes it easier to redo it .
...
To search models it starts with a initial connectivity of the blocks and creates all the combinations of connections given .
...
If the user wants to create their own design matrices, they can use \texttt{symm} and modify the design of the model with \texttt{sbuSymm}.
...
\begin{longtable}[]{@{}lccc@{}}
\caption{\label{tab:puget-model1-2} Relationships between the different blocks on the Puget's dataset for model 1.2. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
...
\caption{\label{tab:puget-model2} Relationships between the different blocks on the Puget's dataset for model 2. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
...
\begin{longtable}[]{@{}lcccc@{}}
\caption{\label{tab:puget-model2-2} Relationships between the different blocks on the Puget's dataset for model 2.2. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
...
\textbf{Invariable} & 0 & 1/3 & 0 & 0 \\
\textbf{Localization} & 1 & 0 & 0 & 0 \\
\bottomrule
\end{longtable}

If we added a superblock with all the data of the different blocks from model 1 we started with the standard relationship between blocks:

\begin{longtable}[]{@{}lcccc@{}}
\caption{\label{tab:puget-model-superblock} Relationships between the different blocks on the Puget's dataset for model superblock. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
...
\textbf{GE} & 0 & 0 & 1 & 0 \\
\textbf{CGH} & 0 & 0 & 1 & 0 \\
\textbf{Superblock} & 1 & 1 & 0 & 1 \\
\textbf{Localization} & 0 & 0 & 1 & 0 \\
\bottomrule
\end{longtable}

But when the best model with the superblock that had highest inner AVE is quite different:

\begin{longtable}[]{@{}lcccc@{}}
\caption{\label{tab:puget-model-superblock-2} Relationships between the different blocks on the Puget's dataset for model superblock.2. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
Model superblock.2 & GE & CGH & Superblock & Localization \\
\midrule
\endfirsthead
\toprule
Model superblock.2 & GE & CGH & Superblock & Localization \\
...
\begin{figure}
\includegraphics{images/pugets_superblock} \caption[Superblock components on Puget's dataset]{First components of the superblock which has all the data of the samples on the Puget's dataset.}\label{fig:puget-superblock}
\end{figure}
...
\begin{figure}
\includegraphics{images/pugets_models} \caption[Different models tried with the same data showing the first components of the CGH data and the transcriptome.]{Different models tried with the same data showing the first components of the CGH data and the transcriptome. Model 1 and 1.2 with transcriptomics, CGH data and all the data about the samples together. Model 2 and 2.2 with transcriptomics, CGH data and all the data bout the samples on different blocks. Model superblock and superblock.2 have all the data in different blokcs and one block with all the data.}\label{fig:puget-models}
\end{figure}
...
\begin{figure}
\includegraphics{images/pugets-weights} \caption[Effect of superblock and weights on the inner AVE on Puget's dataset.]{Effect of superblock and weights on the inner AVE on Puget's dataset. Designs with the superblock showed higher inner AVe scores than without it. Interaction yes/no indicates RNA and CGH interaction.}\label{fig:puget-weights}
\end{figure}

The different models resulted on the following AVE values.
...
\includegraphics{images/hsct-adonis.png}
...
\includegraphics{images/hsct-globaltest.png}

On the microbiome data the results were similar but the p-value was considerably higher but still below the threshold.

\begin{figure}
\includegraphics{images/hsct-ASV-diversity} \caption[Microbiome diversity on HSCT dataset.]{Microbiome diversity on HSCT dataset. On the upper section the Shannon effective and on the lower row the Simpson effective diversity splitted by colon and ileum and controls and CD.}\label{fig:hsct-diversity}
\end{figure}
...
\midrule
\endhead
\textbf{Transcriptome} & 0 & 0 & 0 & 0.6 & 0 \\
\textbf{Microbiome} & 0 & 0 & 0.2 & 0.1 & 0 \\
\textbf{Demographic} & 0 & 0.2 & 0 & 0 & 1 \\
\textbf{Location} & 0.6 & 0.1 & 0 & 0 & 0 \\
Time & 0 & 0 & 1 & 0 & 0 \\
\bottomrule
\end{longtable}
...
\toprule
Model 2.3 & Transcriptome & Microbiome & Demographic & Location & \\
\midrule
\endfirsthead
\toprule
Model 2.3 & Transcriptome & Microbiome & Demographic & Location & \\
...
\textbf{Microbiome} & 0 & 0 & 0.1 & 0.1 & 0 \\
...
The best model of the family 2 confirmed a relationship between the host transcriptome and the location-related variables, while the microbiome was associated with the demographic and location-related variables (see Figure and \href{https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0246367\#pone.0246367.s002}{S2} data of {[}\protect\hyperlink{ref-revilla2021}{197}{]}).
...
\begin{figure}
\centering
\includegraphics{images/paper1_MCIA.png}
\caption{\label{fig:hsct-mcia} MCIA first two synthetic variables on the IBD related cohorts. In red circles the colon and in blue triangles the ileum.}
\end{figure}
...
\centering
\includegraphics{images/paper1_MCIA.png}
\caption{\label{fig:hsct-auc} ROC curve of the different datasets with the models from RGCCA and the result with MCIA. In gray the MCIA method and with discontinued lines the models of RGCCA.}
\end{figure}
...
\begin{longtable}[]{@{}lcccc@{}}
\caption{\label{tab:morgan-model2-3} Relationships between the different blocks on the Morgan's dataset for model 2.3. 0 indicates no relationship and 1 indicates a strong relationship.}\tabularnewline
\toprule
Model 2.3 & Transcriptome & Microbiome & Demographic & Location \\
\midrule
\endfirsthead
...
\begin{figure}
\centering
\includegraphics{images/morgans_models.png}
...
\begin{figure}
\centering
\includegraphics{images/morgans_bootstrap.png}
...
This resulted on the following model:
...
\begin{figure}
\centering
\includegraphics{images/howells_models.png}
...
Current approaches focus on comparing tools on (several) previously published datasets {[}\protect\hyperlink{ref-cantini2021}{202}{]}, an approach that we have taken too.
...
There are reviews of different tools on the same datasets {[}\protect\hyperlink{ref-cantini2020}{205}{]}, some are more theoretical {[}ref:needed{]}, others are focused on a different field {[}\protect\hyperlink{ref-cavill2016}{99}{]}.
...
This assumption is backed up by several other previous studies supporting this relationship or others {[}\protect\hyperlink{ref-thomas1998}{40}, \protect\hyperlink{ref-cornish2008}{41}, \protect\hyperlink{ref-holmes2012}{206}--\protect\hyperlink{ref-brand2021}{208}{]}.
...
PERMANOVA, \texttt{adonis} and \texttt{globaltest} worked well to identify which variables about the samples were important.
But they do not provide variable selection or give any insight into which specific microbial species are driving the association between the microbiome and the variables {[}\protect\hyperlink{ref-susin2020}{209}{]}.\\
...
This also enters in conflict with a principle of data protection: the minimum sufficient information should be collected.
...
The development of the \texttt{experDesign} package helped avoid batch effects due to the \texttt{experDesign} package.
This package might help to improve the quality of the datasets and to expand previously sequenced datasets.
...
As discussed on the related publication, there are several tools already focused on this problem: \texttt{OSAT} ({[}\protect\hyperlink{ref-yan2012}{210}{]}), \texttt{anticlust} ({[}\protect\hyperlink{ref-papenberg2020}{211}{]}) and \texttt{Omixer} ({[}\protect\hyperlink{ref-sinke2021}{212}{]}).
But these tools have some shortcomings, that are covered by \texttt{experDesign}, \texttt{OSAT} cannot handle missing values and does not work well for arbitrary batches.
\texttt{anticlust} only accepts numerical variables but it is based on a mathematical theorem that makes it very powerful.
\texttt{Omixer} has bugs that prevent comparing with the other tools with no possible workaround.

We selected \texttt{MCIA} as a baseline to compare our method because it works well and has a good documentation as well as it is fairly used on the computational world.
The method was developed after \texttt{RGCCA} and recently there have been publications that show that it outperforms other methods on its versatility on different contexts {[}\protect\hyperlink{ref-cantini2021}{202}{]}.
On the dataset analyzed we found it was very competitive but this will be discussed on section \ref{evaluating-models} .

\hypertarget{designing-models}{%
\section{Designing models}\label{designing-models}}

Previous publications using \texttt{RGCCA} on IBD focused on validating inflammation markers genes DUOX2 and APOA1 as inflammation predictors ({[}\protect\hyperlink{ref-tang2017}{80}{]}) from previously published articles {[}\protect\hyperlink{ref-haberman2014}{213}{]}.
Some publication tried to summarize the existing relationships on IBD {[}\protect\hyperlink{ref-hasler_uncoupling_2016}{79}{]}.

Modeling the relationships on \texttt{RGCCA} the connections between blocks had to be modified.
Also what belongs to a block should be modified.
The features selected by \texttt{RGCCA} are highly dependent on which connections are made.
...
To evaluate a model \texttt{RGCCA} provides the average variance explained (AVE), inner and outer.
...
Of the multiple methods available we used \href{https://bioconductor.org/packages/omicade4}{MCIA} {[}\protect\hyperlink{ref-mengMultivariateApproachIntegration2014}{214}{]}.
...
{[}\protect\hyperlink{ref-jiang2019}{215}{]} A paper about multi-omics and microbiome\ldots{}
...
Paper about using transcriptomics to infer inflammation without colonoscopy {[}\protect\hyperlink{ref-ungar2022}{216}{]}
...
Multi-omics seems to focus on metagenomics, metatranscriptomics and metaproteomics and abandon plain 16S sequencing {[}\protect\hyperlink{ref-zhang2019}{217}{]}.

To study complex systems, new technologies like ``organ-on-chip'' or more specifically ``gut-on-chip'' are being developed {[}\protect\hyperlink{ref-collij2021}{218}{]}.
...
\leavevmode\vadjust pre{\hypertarget{ref-human_microbiome_project_consortium_framework_2012}{}}%
...
202. Cantini L, Zakeri P, Hernandez C, Naldi A, Thieffry D, Remy E, et al. \href{https://doi.org/10.1038/s41467-020-20430-7}{Benchmarking joint multi-omics dimensionality reduction approaches for the study of cancer}. Nature Communications. 2021;12:124.
...
210. Yan L, Ma C, Wang D, Hu Q, Qin M, Conroy JM, et al. \href{https://doi.org/10.1186/1471-2164-13-689}{OSAT: A tool for sample-to-batch allocations in genomics experiments}. BMC Genomics. 2012;13:689.

\leavevmode\vadjust pre{\hypertarget{ref-papenberg2020}{}}%
211. Papenberg M, Klau GW. Using anticlustering to partition data sets into equivalent parts. Psychological Methods. 2020. \url{https://doi.org/10.1037/met0000301}.

\leavevmode\vadjust pre{\hypertarget{ref-sinke2021}{}}%
212. Sinke L, Cats D, Heijmans BT. Omixer: Multivariate and reproducible sample randomization to proactively counter batch effects in omics studies. Bioinformatics. 2021. \url{https://doi.org/10.1093/bioinformatics/btab159}.

\leavevmode\vadjust pre{\hypertarget{ref-haberman2014}{}}%
213. Haberman Y, Tickle TL, Dexheimer PJ, Kim M-O, Tang D, Karns R, et al. \href{https://doi.org/10.1172/JCI75436}{Pediatric Crohn disease patients exhibit specific ileal transcriptome and microbiome signature}. The Journal of Clinical Investigation. 2014;124:3617--33.

\leavevmode\vadjust pre{\hypertarget{ref-mengMultivariateApproachIntegration2014}{}}%
214. Meng C, Kuster B, Culhane AC, Gholami AM. \href{https://doi.org/10.1186/1471-2105-15-162}{A multivariate approach to the integration of multi-omics datasets}. BMC Bioinformatics. 2014;15:162.

\leavevmode\vadjust pre{\hypertarget{ref-jiang2019}{}}%
215. Jiang D, Armour CR, Hu C, Mei M, Tian C, Sharpton TJ, et al. \href{https://doi.org/10.3389/fgene.2019.00995}{Microbiome multi-omics network analysis: Statistical considerations, limitations, and opportunities}. Frontiers in Genetics. 2019;10.

\leavevmode\vadjust pre{\hypertarget{ref-ungar2022}{}}%
216. Ungar B, Yavzori M, Fudim E, Picard O, Kopylov U, Eliakim R, et al. Host transcriptome signatures in human faecal-washes predict histological remission in patients with IBD. Gut. 2022. \url{https://doi.org/10.1136/gutjnl-2021-325516}.

\leavevmode\vadjust pre{\hypertarget{ref-zhang2019}{}}%
217. Zhang X, Li L, Butcher J, Stintzi A, Figeys D. \href{https://doi.org/10.1186/s40168-019-0767-6}{Advancing functional and translational microbiome research using meta-omics approaches}. Microbiome. 2019;7:154.

\leavevmode\vadjust pre{\hypertarget{ref-collij2021}{}}%
218. Collij V, Klaassen MAY, Weersma RK, Vila AV. \href{https://doi.org/10.1007/s00439-020-02218-3}{Gut microbiota in inflammatory bowel diseases: moving from basic science to clinical applications}. Human Genetics. 2021;140:703--8.

\leavevmode\vadjust pre{\hypertarget{ref-parkhomenko2009}{}}%
219. Parkhomenko E, Tritchler D, Beyene J. \href{https://doi.org/10.2202/1544-6115.1406}{Sparse Canonical Correlation Analysis with Application to Genomic Data Integration}. Statistical Applications in Genetics and Molecular Biology. 2009;8.

\leavevmode\vadjust pre{\hypertarget{ref-waaijenborg2008}{}}%
220. Waaijenborg S, Hamer PCV de W, Zwinderman AH. \href{https://doi.org/10.2202/1544-6115.1329}{Quantifying the Association between Gene Expressions and DNA-Markers by Penalized Canonical Correlation Analysis}. Statistical Applications in Genetics and Molecular Biology. 2008;7.

\leavevmode\vadjust pre{\hypertarget{ref-witten2009}{}}%
221. Witten DM, Tibshirani RJ. \href{https://doi.org/10.2202/1544-6115.1470}{Extensions of sparse canonical correlation analysis with applications to genomic data}. Statistical Applications in Genetics and Molecular Biology. 2009;8:127.

\leavevmode\vadjust pre{\hypertarget{ref-luxeacao2009}{}}%
222. Lê Cao K-A, Martin PG, Robert-Granié C, Besse P. \href{https://doi.org/10.1186/1471-2105-10-34}{Sparse canonical methods for biological data integration: Application to a cross-platform study}. BMC Bioinformatics. 2009;10:34.

\leavevmode\vadjust pre{\hypertarget{ref-hwang2009}{}}%
223. Hwang H. \href{https://doi.org/10.1007/s11336-009-9119-y}{Regularized Generalized Structured Component Analysis}. Psychometrika. 2009;74:517--30.

\leavevmode\vadjust pre{\hypertarget{ref-soneson2010}{}}%
224. Soneson C, Lilljebjörn H, Fioretos T, Fontes M. \href{https://doi.org/10.1186/1471-2105-11-191}{Integrative analysis of gene expression and copy number alterations using canonical correlation analysis}. BMC Bioinformatics. 2010;11:191.

\leavevmode\vadjust pre{\hypertarget{ref-zhang2011}{}}%
225. Zhang S, Li Q, Liu J, Zhou XJ. \href{https://doi.org/10.1093/bioinformatics/btr206}{A novel computational framework for simultaneous integration of multiple types of genomic data to identify microRNA-gene regulatory modules}. Bioinformatics. 2011;27:i401--9.

\leavevmode\vadjust pre{\hypertarget{ref-lee2011}{}}%
226. Lee W, Lee D, Lee Y, Pawitan Y. \href{https://doi.org/10.2202/1544-6115.1638}{Sparse Canonical Covariance Analysis for High-throughput Data}. Statistical Applications in Genetics and Molecular Biology. 2011;10.

\leavevmode\vadjust pre{\hypertarget{ref-abdi2012}{}}%
227. Abdi H, Williams LJ, Valentin D, Bennani-Dosse M. \href{https://doi.org/10.1002/wics.198}{STATIS and DISTATIS: optimum multitable principal component analysis and three way metric multidimensional scaling}. WIREs Computational Statistics. 2012;4:124--67.

\leavevmode\vadjust pre{\hypertarget{ref-zhang2012}{}}%
228. Zhang S, Liu C-C, Li W, Shen H, Laird PW, Zhou XJ. \href{https://doi.org/10.1093/nar/gks725}{Discovery of multi-dimensional modules by integrative analysis of cancer genomic data}. Nucleic Acids Research. 2012;40:9379--91.

\leavevmode\vadjust pre{\hypertarget{ref-li2012}{}}%
229. Li W, Zhang S, Liu C-C, Zhou XJ. \href{https://doi.org/10.1093/bioinformatics/bts476}{Identifying multi-layer gene regulatory modules from multi-dimensional genomic data}. Bioinformatics. 2012;28:2458--66.

\leavevmode\vadjust pre{\hypertarget{ref-abdi2013}{}}%
230. Abdi H, Williams LJ, Valentin D. \href{https://doi.org/10.1002/wics.1246}{Multiple factor analysis: principal component analysis for multitable and multiblock data sets}. WIREs Computational Statistics. 2013;5:149--79.

\leavevmode\vadjust pre{\hypertarget{ref-schlauch2017}{}}%
231. Schlauch D, Paulson JN, Young A, Glass K, Quackenbush J. \href{https://doi.org/10.1093/bioinformatics/btx139}{Estimating gene regulatory networks with pandaR}. Bioinformatics. 2017;33:2232--4.

\leavevmode\vadjust pre{\hypertarget{ref-ray2014}{}}%
232. Ray P, Zheng L, Lucas J, Carin L. \href{https://doi.org/10.1093/bioinformatics/btu064}{Bayesian joint analysis of heterogeneous genomics data}. Bioinformatics. 2014;30:1370--6.

\leavevmode\vadjust pre{\hypertarget{ref-bunte2016}{}}%
233. Bunte K, Leppäaho E, Saarinen I, Kaski S. \href{https://doi.org/10.1093/bioinformatics/btw207}{Sparse group factor analysis for biclustering of multiple data sources}. Bioinformatics. 2016;32:2457--63.

\leavevmode\vadjust pre{\hypertarget{ref-chen2013}{}}%
234. Chen M, Gao C, Ren Z, Zhou HH. \href{http://arxiv.org/abs/1311.6186}{Sparse CCA via precision adjusted iterative thresholding}. arXiv:13116186 {[}math, stat{]}. 2013.

\leavevmode\vadjust pre{\hypertarget{ref-leppuxe4aho2017}{}}%
235. Leppäaho E, Ammad-ud-din M, Kaski S. \href{http://jmlr.org/papers/v18/16-509.html}{GFA: Exploratory analysis of multiple data sources with group factor analysis}. Journal of Machine Learning Research. 2017;18:1--5.

\leavevmode\vadjust pre{\hypertarget{ref-klami2014}{}}%
236. Klami A, Bouchard G, Tripathi A. \href{http://arxiv.org/abs/1312.5921}{Group-sparse embeddings in collective matrix factorization}. arXiv:13125921 {[}cs, stat{]}. 2014.

\leavevmode\vadjust pre{\hypertarget{ref-meng2018}{}}%
237. Meng C, Basunia A, Peters B, Gholami AM, Kuster B, Culhane AC. \href{https://www.biorxiv.org/content/10.1101/046904v2}{MOGSA: integrative single sample gene-set analysis of multiple omics data}. 2018.

\leavevmode\vadjust pre{\hypertarget{ref-yang2016b}{}}%
238. Yang Z, Michailidis G. \href{https://doi.org/10.1093/bioinformatics/btv544}{A non-negative matrix factorization method for detecting modules in heterogeneous omics multi-modal data}. Bioinformatics. 2016;32:1--8.

\leavevmode\vadjust pre{\hypertarget{ref-zhao2015}{}}%
239. Zhao S, Gao C, Mukherjee S, Engelhardt BE. \href{http://arxiv.org/abs/1411.2698}{Bayesian group latent factor analysis with structured sparsity}. arXiv:14112698 {[}q-bio, stat{]}. 2015.

\leavevmode\vadjust pre{\hypertarget{ref-voillet2016}{}}%
240. Voillet V, Besse P, Liaubet L, San Cristobal M, González I. \href{https://doi.org/10.1186/s12859-016-1273-5}{Handling missing rows in multi-omics data integration: Multiple imputation in multiple factor analysis framework}. BMC Bioinformatics. 2016;17:402.

\leavevmode\vadjust pre{\hypertarget{ref-beaton2016}{}}%
241. Beaton D, Dunlop J, Abdi H, Alzheimer's Disease Neuroimaging Initiative. \href{https://doi.org/10.1037/met0000053}{Partial least squares correspondence analysis: A framework to simultaneously analyze behavioral and genetic data}. Psychological Methods. 2016;21:621--51.

\leavevmode\vadjust pre{\hypertarget{ref-singh2019}{}}%
242. Singh A, Shannon CP, Gautier B, Rohart F, Vacher M, Tebbutt SJ, et al. \href{https://doi.org/10.1093/bioinformatics/bty1054}{DIABLO: An integrative approach for identifying key molecular drivers from multi-omics assays}. Bioinformatics. 2019;35:3055--62.

\leavevmode\vadjust pre{\hypertarget{ref-yoon2019}{}}%
243. Yoon G, Carroll RJ, Gaynanova I. \href{http://arxiv.org/abs/1807.05274}{Sparse semiparametric canonical correlation analysis for data of mixed types}. arXiv:180705274 {[}stat{]}. 2019.

\leavevmode\vadjust pre{\hypertarget{ref-gaynanova2017}{}}%
244. Gaynanova I, Li G. \href{http://arxiv.org/abs/1707.06573}{Structural learning and integrative decomposition of multi-view data}. arXiv:170706573 {[}stat{]}. 2017.

\leavevmode\vadjust pre{\hypertarget{ref-madrigal2017}{}}%
245. Madrigal P. \href{https://doi.org/10.1093/bioinformatics/btw724}{fCCAC: Functional canonical correlation analysis to evaluate covariance between nucleic acid sequencing datasets}. Bioinformatics. 2017;33:746--8.

\leavevmode\vadjust pre{\hypertarget{ref-yoshida2017}{}}%
246. Yoshida K, Yoshimoto J, Doya K. \href{https://doi.org/10.1186/s12859-017-1543-x}{Sparse kernel canonical correlation analysis for discovery of nonlinear interactions in high-dimensional data}. BMC Bioinformatics. 2017;18:108.

\leavevmode\vadjust pre{\hypertarget{ref-kawaguchi2017}{}}%
247. Kawaguchi A, Yamashita F. \href{https://doi.org/10.1093/biostatistics/kxx011}{Supervised multiblock sparse multivariable analysis with application to multimodal brain imaging genetics}. Biostatistics. 2017;18:651--65.

\leavevmode\vadjust pre{\hypertarget{ref-feng2018}{}}%
248. Feng Q, Jiang M, Hannig J, Marron JS. \href{http://arxiv.org/abs/1704.02060}{Angle-based joint and individual variation explained}. arXiv:170402060 {[}stat{]}. 2018.

\leavevmode\vadjust pre{\hypertarget{ref-argelaguet2020}{}}%
249. Argelaguet R, Arnol D, Bredikhin D, Deloro Y, Velten B, Marioni JC, et al. \href{https://doi.org/10.1186/s13059-020-02015-1}{MOFA+: A statistical framework for comprehensive integration of multi-modal single-cell data}. Genome Biology. 2020;21:111.

\leavevmode\vadjust pre{\hypertarget{ref-brown2018}{}}%
250. Brown BC, Bray NL, Pachter L. Expression reflects population structure. 2018. \url{https://doi.org/10.1101/364448}.

\leavevmode\vadjust pre{\hypertarget{ref-zhang2020}{}}%
251. Zhang Y, Gaynanova I. \href{http://arxiv.org/abs/1811.08511}{Joint association and classification analysis of multi-view data}. arXiv:181108511 {[}cs, stat{]}. 2020.

\leavevmode\vadjust pre{\hypertarget{ref-tang2021}{}}%
252. Tang TM, Allen GI. \href{http://arxiv.org/abs/1810.00832}{Integrated principal components analysis}. arXiv:181000832 {[}stat{]}. 2021.

\leavevmode\vadjust pre{\hypertarget{ref-min2019}{}}%
253. Min EJ, Safo SE, Long Q. \href{https://doi.org/10.1093/bioinformatics/bty726}{Penalized co-inertia analysis with applications to -omics data}. Bioinformatics (Oxford, England). 2019;35:1018--25.

\leavevmode\vadjust pre{\hypertarget{ref-safo2018}{}}%
254. Safo SE, Li S, Long Q. \href{https://doi.org/10.1111/biom.12715}{Integrative analysis of transcriptomic and metabolomic data via sparse canonical correlation analysis with incorporation of biological information}. Biometrics. 2018;74:300--12.

\leavevmode\vadjust pre{\hypertarget{ref-min2017}{}}%
255. Min W, Liu J, Zhang S. \href{http://arxiv.org/abs/1710.04792}{Sparse weighted canonical correlation analysis}. arXiv:171004792 {[}cs, stat{]}. 2017.

\leavevmode\vadjust pre{\hypertarget{ref-bouhaddani2018}{}}%
256. Bouhaddani S el, Uh H-W, Jongbloed G, Hayward C, Klarić L, Kiełbasa SM, et al. \href{https://doi.org/10.1186/s12859-018-2371-3}{Integrating omics datasets with the OmicsPLS package}. BMC Bioinformatics. 2018;19:371.

\leavevmode\vadjust pre{\hypertarget{ref-pimentel2018}{}}%
257. Pimentel H, Zhiyue H, Huang H. \href{https://doi.org/10.1007/s40484-017-0127-0}{Biclustering by sparse canonical correlation analysis}. 2018;6:11.

\leavevmode\vadjust pre{\hypertarget{ref-kim2019}{}}%
258. Kim Y, Bismeijer T, Zwart W, Wessels LFA, Vis DJ. \href{https://doi.org/10.1038/s41467-019-13027-2}{Genomic data integration by WON-PARAFAC identifies interpretable factors for predicting drug-sensitivity in vivo}. Nature Communications. 2019;10:5034.

\leavevmode\vadjust pre{\hypertarget{ref-lock2020}{}}%
259. Lock EF, Park JY, Hoadley KA. \href{http://arxiv.org/abs/2002.02601}{Bidimensional linked matrix factorization for pan-omics pan-cancer analysis}. arXiv:200202601 {[}cs, q-bio, stat{]}. 2020.

\leavevmode\vadjust pre{\hypertarget{ref-ronen2019}{}}%
260. Ronen J, Hayat S, Akalin A. \href{https://doi.org/10.26508/lsa.201900517}{Evaluation of colorectal cancer subtypes and cell lines using deep learning}. Life Science Alliance. 2019;2.

\leavevmode\vadjust pre{\hypertarget{ref-shi2019}{}}%
261. Shi WJ, Zhuang Y, Russell PH, Hobbs BD, Parker MM, Castaldi PJ, et al. \href{https://doi.org/10.1093/bioinformatics/btz226}{Unsupervised discovery of phenotype-specific multi-omics networks}. Bioinformatics. 2019;35:4336--43.

\leavevmode\vadjust pre{\hypertarget{ref-csala2020}{}}%
262. Csala A, Zwinderman AH, Hof MH. \href{https://doi.org/10.1186/s12859-019-3286-3}{Multiset sparse partial least squares path modeling for high dimensional omics data analysis}. BMC Bioinformatics. 2020;21:9.

\leavevmode\vadjust pre{\hypertarget{ref-fan2020}{}}%
263. Fan Z, Zhou Y, Ressom HW. \href{https://doi.org/10.3390/metabo10040144}{MOTA: Network-Based Multi-Omic Data Integration for Biomarker Discovery}. Metabolites. 2020;10:144.

\leavevmode\vadjust pre{\hypertarget{ref-shu2020}{}}%
264. Shu H, Wang X, Zhu H. \href{https://doi.org/10.1080/01621459.2018.1543599}{D-CCA: A decomposition-based canonical correlation analysis for high-dimensional datasets}. Journal of the American Statistical Association. 2020;115:292--306.

\leavevmode\vadjust pre{\hypertarget{ref-hawinkel2020}{}}%
265. Hawinkel S, Bijnens L, Cao K-AL, Thas O. \href{https://doi.org/10.1093/nargab/lqaa050}{Model-based joint visualization of multiple compositional omics datasets}. NAR Genomics and Bioinformatics. 2020;2.

\leavevmode\vadjust pre{\hypertarget{ref-gundersen2020}{}}%
266. Gundersen G, Dumitrascu B, Ash JT, Engelhardt BE. \href{https://proceedings.mlr.press/v115/gundersen20a.html}{Uncertainty in Artificial Intelligence}. PMLR; 2020. p. 945--55.

\leavevmode\vadjust pre{\hypertarget{ref-velten2020}{}}%
267. Velten B, Braunger JM, Arnol D, Argelaguet R, Stegle O. \href{https://doi.org/10.1101/2020.11.03.366674}{Identifying temporal and spatial patterns of variation from multi-modal data using MEFISTO}. bioRxiv. 2020;2020.11.03.366674.
...
\midrule
\endhead
SCCA & {[}\protect\hyperlink{ref-parkhomenko2009}{219}{]} \\
PCCA & {[}\protect\hyperlink{ref-waaijenborg2008}{220}{]} \\
PMA & {[}\protect\hyperlink{ref-witten2009}{221}{]} \\
sPLS & {[}\protect\hyperlink{ref-luxeacao2009}{222}{]} \\
gesca & {[}\protect\hyperlink{ref-hwang2009}{223}{]} \\
Regularized dual CCA & {[}\protect\hyperlink{ref-soneson2010}{224}{]} \\
RGCCA & {[}\protect\hyperlink{ref-tenenhaus_regularized_2011}{113}{]} \\
SNMNMF & {[}\protect\hyperlink{ref-zhang2011}{225}{]} \\
scca & {[}\protect\hyperlink{ref-lee2011}{226}{]} \\
STATIS & {[}\protect\hyperlink{ref-abdi2012}{227}{]} \\
joint NMF & {[}\protect\hyperlink{ref-zhang2012}{228}{]} \\
sMBPLS & {[}\protect\hyperlink{ref-li2012}{229}{]} \\
Bayesian group factor analysis & {[}\protect\hyperlink{ref-virtanen2012}{132}{]} \\
RIMBANET & {[}\protect\hyperlink{ref-zhu2012}{126}{]} \\
FactoMineR & {[}\protect\hyperlink{ref-abdi2013}{230}{]} \\
JIVE & {[}\protect\hyperlink{ref-lock2013a}{134}{]} \\
pandaR & {[}\protect\hyperlink{ref-schlauch2017}{231}{]} \\
omicade4 & {[}\protect\hyperlink{ref-meng2014}{161}{]} \\
STATegRa & {[}\protect\hyperlink{ref-planell2021}{162}{]} \\
Joint factor model & {[}\protect\hyperlink{ref-ray2014}{232}{]} \\
GFAsparse & {[}\protect\hyperlink{ref-bunte2016}{233}{]} \\
Sparse CCA & {[}\protect\hyperlink{ref-chen2013}{234}{]} \\
CCAGFA & {[}\protect\hyperlink{ref-leppuxe4aho2017}{235}{]} \\
CMF & {[}\protect\hyperlink{ref-klami2014}{236}{]} \\
MOGSA & {[}\protect\hyperlink{ref-meng2018}{237}{]} \\
iNMF & {[}\protect\hyperlink{ref-yang2016b}{238}{]} \\
BASS & {[}\protect\hyperlink{ref-zhao2015}{239}{]} \\
imputeMFA & {[}\protect\hyperlink{ref-voillet2016}{240}{]} \\
PLSCA & {[}\protect\hyperlink{ref-beaton2016}{241}{]} \\
mixOmics & {[}\protect\hyperlink{ref-singh2019}{242}{]} \\
mixedCCA & {[}\protect\hyperlink{ref-yoon2019}{243}{]} \\
SLIDE & {[}\protect\hyperlink{ref-gaynanova2017}{244}{]} \\
fCCAC & {[}\protect\hyperlink{ref-madrigal2017}{245}{]} \\
TSKCCA & {[}\protect\hyperlink{ref-yoshida2017}{246}{]} \\
SMSMA & {[}\protect\hyperlink{ref-kawaguchi2017}{247}{]} \\
AJIVE & {[}\protect\hyperlink{ref-feng2018}{248}{]} \\
MOFA & {[}\protect\hyperlink{ref-argelaguet2020}{249}{]} \\
PCA+CCA & {[}\protect\hyperlink{ref-brown2018}{250}{]} \\
JACA & {[}\protect\hyperlink{ref-zhang2020}{251}{]} \\
iPCA & {[}\protect\hyperlink{ref-tang2021}{252}{]} \\
pCIA & {[}\protect\hyperlink{ref-min2019}{253}{]} \\
sSCCA & {[}\protect\hyperlink{ref-safo2018}{254}{]} \\
SWCCA & {[}\protect\hyperlink{ref-min2017}{255}{]} \\
OmicsPLS & {[}\protect\hyperlink{ref-bouhaddani2018}{256}{]} \\
SCCA-BC & {[}\protect\hyperlink{ref-pimentel2018}{257}{]} \\
WON-PARAFAC & {[}\protect\hyperlink{ref-kim2019}{258}{]} \\
BIDIFAC & {[}\protect\hyperlink{ref-lock2020}{259}{]} \\
maui & {[}\protect\hyperlink{ref-ronen2019}{260}{]} \\
SmCCNet & {[}\protect\hyperlink{ref-shi2019}{261}{]} \\
msPLS & {[}\protect\hyperlink{ref-csala2020}{262}{]} \\
MOTA & {[}\protect\hyperlink{ref-fan2020}{263}{]} \\
D-CCA & {[}\protect\hyperlink{ref-shu2020}{264}{]} \\
COMBI & {[}\protect\hyperlink{ref-hawinkel2020}{265}{]} \\
DPCCA & {[}\protect\hyperlink{ref-gundersen2020}{266}{]} \\
MEFISTO & {[}\protect\hyperlink{ref-velten2020}{267}{]} \\
...
\href{https://doi.org/10.1371/journal.pone.0246367}{Available online} \textwidth \textheight
...
\href{https://joss.theoj.org/papers/10.21105/joss.03358}{Available online}
...
\begin{figure}
\includegraphics{images/barcelona-diversity} \caption[Diversity indices of Barcelona according to the location and disease status.]{Diversity indices of Barcelona according to the location and disease status. There is a lot of diversity between different groups but importantly the control samples overlap with the patients with inflammatory bowel disease.}\label{fig:barcelona-diversity}
\end{figure}
