...
\textbf{Introduction}: Inflammatory bowel disease is a complex intestinal disease with several factors that affect each other.
The origin and process that maintains the disease is not fully understood.
There are some clues that microbiome is related to starting or maintaining this disease.
Finding relationships between the microbiome and the gastrointestinal mucosa could help advance treatments, diagnosis or prevent it.

\textbf{Methods}: We based our analysis on intestinal bacterial 16S and human transcriptome data from biopsies from multiple timepoints and locations of the intestine.
We extended regularized generalized canonical correlation analysis to find models that are coherent with previous knowledge on the disease taking into account samples' information.
The results were compared with multiple co-inertia analysis and evaluated according to how well they can classify the samples on their location.
Multiple datasets of patients on different treatments and conditions were analysed.

\textbf{Results}: Splitting samples variables into different blocks results on models of these relationships that show differences on the genes and microorganismes selected.
These models of our new method inteRmodel outperformed multiple co-inertia analysis to classify the samples according to their location.
Despite being used on datasets of different sources the resulting models show similar relationship strength between the variables.

\textbf{Discussion}: Our method finds how strong are the relationships of microbiome, transcriptome and other variables.
...
These bacteria are known to be more abundant on patients with IBD {[}ref:needed{]}.
...
Caution and a good relationship with the sample and data generator is important to discover this kind of problems and ensure the quality of the data.
...
There are reviews that apply different tools to the same datasets {[}\protect\hyperlink{ref-cantini2021}{129}{]}, some are more theoretical {[}\protect\hyperlink{ref-bersanelli2016}{206}{]}, others are focused on a different field {[}\protect\hyperlink{ref-cavill2016}{99}{]}.
...
This is especially important due to the lack of golden datasets or a way to reliable simulate datasets as discussed on the previous section {[}\protect\hyperlink{ref-ref}{\textbf{ref?}}{]}(discussion-datasets).
...
We briefly considered using \texttt{STATegRa}. However, it is not possible to select which interactions between the blocks exists.
...
This differs to other publications that relied on them to draw their conclusions {[}\protect\hyperlink{ref-huxe4sler2017}{214}{]}.
...
Some publication tried to summarize the existing relationships in IBD {[}\protect\hyperlink{ref-hasler_uncoupling_2016}{79}{]}, but none were focused on finding the relationships on IBD using \texttt{RGCCA} as we did.
...
As the inner AVE measures how good does the data fit in the model, it makes more sense to pay more attention to it.
...
It is known that the mucosal transciptome is related to sample localization (of)i.e.~small or large intestine) and disease activity {[}\protect\hyperlink{ref-criss2021}{228}{]}.
...
In healthy humans, it has been suggested that there is a shared microbial patterns among groups of individuals {[}\protect\hyperlink{ref-arumugam2011}{229}{]}.
Such theory proposes three groups of similar microbiota composition, named enterotypes.
However, the enterotypes classification is not unanimously accepted {[}\protect\hyperlink{ref-koren2013}{230}, \protect\hyperlink{ref-cheng2019}{231}{]}.
...
Even in healthy non-IBD patients, multiple samples with microbiota from the same patients could provide a common microbiome background, which can help to identify core individual microbiome and altered microbiome states later on.
...
There seems to be a tendency of multi-omics project to focus on metagenomics, metatranscriptomics and metaproteomics and abandon plain 16S sequencing {[}\protect\hyperlink{ref-zhang2019}{239}{]}.
...
There is some evidence that currently literature focus on already known and studied genes instead of more novel and with higher relevance genes {[}\protect\hyperlink{ref-haynes2018}{241}{]}.
