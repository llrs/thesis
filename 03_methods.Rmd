...
This test is implemented in the function `wilcox.test` from the stats package in R.
...
$rank(g_j)=r_j$ where each $g$ is a gene.
$p$ is a
...
The usual value used is the enrichment score (ES) defined as: $ES=max(|P_{hit}(S, i)-P_{miss}(S, i)|)$ to accurately calculate the run, a high number of permutations are required.
However, when more than one pathway ($S$) is evaluated those values must be normalized by dividing it by the mean of all the ES.
