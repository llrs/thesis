...
The glioma dataset came from diffuse intrinsic pontine glioma patients that included the host transcriptome analyzed with Agilent 44K Whole Human Genome Array G4410B and G4112F, patients copy number variation was processed with the ADM-2 algorithm, and data from comparative genomic hybridization (CGH) analyzed using [Mutation Surveyor software](https://www.softgenetics.com/mutationSurveyor.php "Website of the software").
...
Later, those samples with good enough quality as recommended by [FastQC](https://www.bioinformatics.babraham.ac.uk/projects/fastqc/ "FastQC website") afterwards [cutadapt](https://github.com/marcelm/cutadapt "cutadapt repository") (version 1.7.1) [@martin2011] was used for quality filtering and the libraries were mapped against the human reference genome using the [STAR aligner](https://github.com/alexdobin/STAR "STAR repository") (2.5.2a) with Ensembl annotation (release GRCh38.p10) [@dobin2013].
...
The Wilcoxon Rank Sum test called Mann-Whitney test on two sample data is the non-parametric version of the T-test when the interest is to test if both medians (M) are equal or not.
...
Being the same samples $X = X_1 , …, X_m , Y = Y_1 , …, Y_n$ and if we want to test if $M_X = M_Y$ (M =median), we sort all the data and $j$ is the rank.
...
$$
I_j = \begin{cases}
1 & \text{if observation with rank j comes from X} \\
0 & \text{if observation with rank j comes from Y} \\
\end{cases}
$$

The exact distribution of $T_x$ is tabulated for each value of $m$ and $n$.
Furthermore, the asymptotic distribution of $T_X$ is normal with parameters:

$$
E(T_X) = \dfrac{m(m+n+1)}{2}, V(T_X)=\dfrac{mn(m+n+1)}{12}
$$

This test or similar are used by clusterProfiler as implemented in the function `wilcox.test` from the stats package in R.
...
On pseudocode the core of the program can be defined as:
...
It needs to have a complete case with no missing values (at the time of writing this thesis there is a new development version not released on CRAN[^methods-3] yet that replaces any missing value by a 0) and time is not considered as a special variable.
