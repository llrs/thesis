...
To test if some variables show an unexpected importance according to a statistic like fold change or value [fgsea](https://bioconductor.org/packages/fgsea) [@korotkevich2021] methods was used.
...
[GSVA](https://bioconductor.org/packages/GSVA) is a method related to [fgsea](https://bioconductor.org/packages/fgsea) that summarize the variables' numerical value so that they can be later used by other methods [@hänzelmann2013].
...
[clusterProfiler](https://bioconductor.org/packages/clusterProfiler): test genes enrichment for functionality based on information on pathway databases [@wu2021].
