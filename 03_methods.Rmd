...
<!-- # TODO cite articles using these methods -->
...
<!-- # TODO Organize better this section -->
...
Under the standard fuzzy set logic a set $S$ is a group of elements for which each element $e$ has an $\alpha$ membership to that set.
...
[^methods-2]: Technical replicates are samples that are measured multiple times to ensure the accuracy of the measurement.
...
<!-- # TODO add flowchart from analysis process like http://mixomics.org/mixdiablo/ {1 and 2} -->

## Regularized generalized canonical correlation analysis

<!-- # TODO Extend explanation introducing dot product -> PCA -> CCA -> RGCCA from a review or something-->

The regularized generalized canonical correlation analysis is an extension to the canonical correlation analysis [@jordan1875; @hotelling1936] which is an old method to find agreement between two, or more, scorers (as it was first introduced on the literature) or sources.
...
Previously canonical correlation methods did not have the ability to integrate more than 2 sources of data as does RGCCA [@R-RGCCA].
...
The method to perform the dimensional reduction using the sparse method consists on maximizing this function:
...
The next models analyzed consists on the relationships between the two experimental blocks and them with all the metadata of the samples, these are models of family 1 denoted by 1.Y, where Y change as different relationships between the three blocks change. 
