...
Samples for the different cohorts collected on the Hospital Clinic were collected similarly, differences are described below together with a brief description of the different datasets.
...
On the BARCELONA dataset the data was processed using 16S-V3V4 primers

(Macrogen: 341f/785r) pair 341f/806r on a MiSeq Nano sequencing bcl2fastq (v1.8.4).

341f: 5'-CCTACGGGAGGCAGCAG-3'

515f: 5'-GTGCCAGCMGCCGCGGTAA (test amplification michigan)

806r: 5'-GGACTACHVHHHTWTCTAAT-3'

806r: 5'-GGACTACHVGGGTWTCTAAT-3' (test amplification michigan)
...
[experDesign](https://cran.r-project.org/package=experDesign) is a package that I implemented to help designing an experiment before sequencing the samples.
...
It needs to have a complete case with no missing values (at the time of writing this thesis there is a new development version that replaces any missing value by a 0) and time is not considered as a special variable.
...
The smaller the $s_j$, the larger the degree of sparsity for $a_j$.The values of $s_j$ were estimated using \[@schäfer2005\] when the block was from 16S data or RNaseq, otherwise 1 was used.
...
To help find the fitting model for the data I created an R package, named [inteRmodel](https://llrs.github.io/inteRmodel/), which helps finding the right model and how fit it is for your data via a bootstrapping procedure.
