...
The protocol was approved by the Institutional Ethics Committee of the Hospital Clinic de Barcelona (Study Number HCB/2012/7956).
...
<!--# Michigan not used diversity weird on controls vs disease samples-->
...
Together with Qiime2 [@bolyen2019] it is an alternative to OTUs which allows to be able to compare results between studies because it doesn't summarize together any sequence and provides more resolution to differences on the fragment amplified.

## Statistics

Comparisons between different conditions of samples of the datasets were done following the limma-voom method \[@ritchie2015; @law2014\] after being transformed via the edgeR package \[@mccarthy2012\] following the workflow described on [@law2018].
...
Usually it is performed with the fisher test or the hypergeometric test or the proportion test.
...
Gene Set Enrichment Analysis (GSEA) is a computational method originally developed to determine whether an a priori defined set of genes shows statistically significant and concordant differences between two biological states.
...
[clusterProfiler](https://bioconductor.org/packages/clusterProfiler): test genes enrichment for functionality based on information on pathway databases [@wu2021] using the over representative analysis approach.
...
Pathways of genes from the REACTOME database were tested on the weight of different models [@fabregat2016].

[GSVA](https://bioconductor.org/packages/GSVA) is a method that summarize the variables' numerical value changing the space of $variable\text{ x }sample$ to $group\text{ x }sample$ [@hänzelmann2013].
This enables other methods to use this new space instead of the original variables.
GSVA was used to estimate the expression of the pathways (again from the REACTOME database) and try to find the relationships between the pathways and the microbiome at different taxonomic levels.

### Other methods used

[vegan](https://cran.r-project.org/package=vegan) package provides the adonis method used to test if microbiome data variance is due to other variables [@oksanen2020].
...
[globaltest](https://bioconductor.org/packages/globaltest) is a package with methods for testing complex hypothesis and help decide if the variables were influencing [@goeman2006].
...
We looked to see if using some co-expression measure between the microbiome and the RNAseq would help identify relationships.
...
It was used as a baseline method to find relationships between microorganisms and genes from the package [omicade4](https://bioconductor.org/packages/omicade4).

[STATegRa](https://bioconductor.org/packages/STATegRa), is a framework for integrating datasets with two data types using parametric and non-parametric methods [@planell2021].
It was used to explore the datasets and see what do they had in common.

[BaseSet](https://cran.r-project.org/package=BaseSet) is a package that I implements fuzzy set logic for easy calculation of probability to belong to a group.
...
[experDesign](https://cran.r-project.org/package=experDesign) is a package that I implemented to help designing an experiment before sequencing the samples [@revillasancho2021].
It also helps to quantify if a given experiment has batch effect due to the batches used to measure the values.
It was used to avoid batch effect on the sequencing of the BARCELONA dataset.

Regularized generalized canonical correlation analysis, [RGCCA](https://cran.r-project.org/package=RGCCA), has been the main method used on this thesis [@tenenhaus2017a].
...
Models were further validated using a bootstrap approach to measure thier accuracy and likelihood on the data available.
