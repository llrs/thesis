...
When there were differences they are noted on that specific dataset sub-section.
...
The glioma dataset is the data provided as an example by the authors of RGCCA from a previous publication [@puget2012].
...
Biopsies were obtained endoscopically during routine diangosis RNA was extracted.
DNA from the 16S rRNA gene was amplified with primers 319F and 806R.
Both RNA and DNA was then sequenced on HiSeq 2000 and MiSeq.
...
$$
\begin{equation} 
 \(\text{X model: } X = TW ^ T + T_{Y-ortho} P ^ T_{Y-ortho} + E) \\ 
 \(\text{Y model: } Y = UC ^ T + U_{X-ortho} P ^ T_{X-ortho} + F) \\
...
To better understand the regularized canonical correlation analysis first an understanding of the principal component analysis is required.
Later a brief description of the canonical correlation method will be used to finally explain the regularized generalized canonical correlation.

Principal component analysis defines a new orthogonal coordinate system that optimally describes variance in a single dataset with decreasing variance.
It does so by decomposing the numerical matrix into the eigenvalues and eigenvectors.
...
Typically data is normalized and standardized to avoid that a variable with higher variance dominates the results.
...
It extends the PCA to a two sources of data and continues to provide the same output new variables and weights of the existing variables indicating their importance.
...
The method to perform the dimensional reduction using the sparse method consists on maximizing the same equation but with a different constraint: Specifically, RGCCA with all $\tau_j = 1$ equal to 1 is combined with an L1-penalty that gives rise to SGCCA
...
The $s_j$ controls the sparsity estimated of the data, the smaller, the larger is the degree of sparsity of $X_j$.
...
As previously explained, there are different $g$ functions that could be used but the centroid method was chosen to detect both positive and negative relationships without more computational costs.
...
Other methods related are partial least squares (PLS) regression, which maximizes covariance with data variation of 1 and shrinkage of 1, canonical correlation analysis equivalent to data variation equal to 1 and shrinkage of 0, and redundancy analysis which one block weights normalized value is 1 and the other its the variance that is equal to 1.
...
The next models analyzed consists on the relationships between the two experimental blocks and them with all the metadata of the samples, these are models of family 1 denoted by 1.Y, where Y changes as different relationships between the three blocks change.
...
These models are from the family 2 denoted by the name 2.Y, where 2 denotes the family of the model and Y change as different relationships between the three blocks change.
...
Most of them are based on an over representation analysis, where a group of elements is tested whether they are present on other group.
...
It has been widely used since its original publication including on IBD [@protiva2016].
...
It can take into account spatial distribution provide which technical replicates[^methods-3] are best given the number of samples that fit on a batch.
