...
The glioma dataset is the data provided as an example of biological data by the authors of \acr{RGCCA} from a previous publication [@puget2012].
...
The copy number variation of the samples was processed with the ADM-2 algorithm, and data from \acr{CGH} analyzed using [Mutation Surveyor software](https://www.softgenetics.com/mutationSurveyor.php "Website of the software").
...
: (#tab:Puget) Characteristics of samples from the Puget dataset.
...
Samples from the \acr{HSCT} dataset used in this thesis were from a cohort of patients with severe refractory \acr{CD} undergoing \acr{HSCT}.
...
All patients provided written consent following extensive counselling about being included on the study and using their data on publications. Variables collected include: sex, age at diagnosis, duration of the disease, age, disease status, sample location, local simple endoscopic score and \acr{CDAI}.

Colonic and ileal biopsies were obtained at several time points during ileo-colonoscopy, at inclusion and every 6 or 12 months after \acr{HSCT} up to 4 years after the start of the treatment.
Samples were obtained when possible from both uninvolved and involved areas.
In addition, biopsies were taken from the ileum and colon regions of 19 non-\acr{IBD} controls consisting of individuals with no history of \acr{IBD} and who presented no significant pathological findings following endoscopic examination for colon cancer surveillance (Hospital Univesitari Mútua de Terrassa --Spain--).
...
| Disease status (non-\acr{IBD}/\acr{CD})              | 51/107      |
| Sample location (ileum/colon/unknown)                | 48/108/2    |
| Local simple endoscopic score for \acr{CD}: mean (min-max) | 2.15 (0-12) |
| \acr{CDAI}: mean (min-max)                                 | 120 (0-450  |

: (#tab:HSCT) Characteristics of samples from the HSCT dataset.

### Häsler's dataset {#methods-hasler}

An \acr{IBD}-related dataset was obtained by Prof. Dr. Rosentiel and Prof. Dr. Robert Häsler [@hasler_uncoupling_2016].
Biopsies were obtained endoscopically during routine diagnosis RNA and DNA were extracted using standard procedures.
DNA from the \acr{16S} gene was amplified with primers 319F and 806R.
Both RNA and DNA was then sequenced on HiSeq 2000 and MiSeq respectively.
These biopsies included samples from the terminal ileum and sigma from \acr{CD}, \acr{UC}, infectious disease-controls and healthy non-\acr{IBD}.
...
|:------------------------------|:--------:|
| Disease status (non-\acr{IBD}/\acr{IBD})  | 33/26    |
| Sex (female/male)             | 42/17    |
| Sample location (ileum/colon) | 30/29    |

: (#tab:Hasler) Characteristics of samples from the Häsler dataset.

### Morgan's dataset {#methods-morgan}

A previously published dataset from a pouchitis study was analyzed [@morgan2015].
In this study patients having undergone proctocolectomy with ileal pouch-anal anastomosis for treatment of \acr{UC} or familial adenomatous polyposis at least 1 year prior to enrollment were recruited at Mount Sinai Hospital (Toronto, Canada) excluding individuals with a diagnosis of \acr{CD}.
...
This dataset included anonymous identifiers for patients, whether the sample was from the \acr{PPI} or from the pouch, the sex, the outcome of the procedure and an inflammation score.
...
| Sample location (Pouch/\acr{PPI}) | 59/196   |

: (#tab:Morgan) Characteristics of samples from the Morgan dataset.

### Howell's dataset {#methods-howell}

This dataset included a cohort of 66 treatment-naïve children at diagnosis of their \acr{IBD}, along with 30 age- and sex-matched non-inflammatory control children, recruited at the Paediatric Gastroenterology unit at Addenbrooke's Hospital (England) [@howell2018].

Data from 77 samples that had both RNAseq and \acr{16S} data was used.
There are 10 non-\acr{IBD} samples, 11 with \acr{CD} and 11 with \acr{UC}.
...
|:------------------------------------------------|:---------:|
| Disease (non-\acr{IBD}/\acr{CD}/\acr{UC})                         | 11/10/11  |
...
: (#tab:howell) Characteristics of samples from the Howell dataset.
...
Biopsies from the  \acr{CD} cohort were resuspended in 180 $\mu$l TET (TrisHCl 0.02M, EDTA 0.002M, Triton 1X) buffer and 20mg/ml lysozyme (Carl Roth, Quimivita, S.A.). Samples were incubated for 1h at 37°C and vortexed with 25 $\mu$l Proteinase K before incubating at 56°C for 3h.
...
Briefly, the V3-V4 regions of \acr{16S} gene were amplified (15x15 cycles) following a previously described two-step protocol [@klindworth2013] using forward and reverse primers 341F-ovh/785R-ovh [@edgar2013].
...
For the  dataset the processing of raw-reads was performed by using the [IMNGS](https://www.imngs.org/ "IMNGS website") (version 1.0 Build 2007) [@lagkouvardos2016] pipeline based on the [UPARSE](https://www.drive5.com/uparse/ "UPARSE website") approach [@edgar2013].
...
\acr{OTUs} were clustered at 97% sequence similarity.
...
The microbiome was visually inspected for batch effects in \acr{PCA}; none were found.
The resulting \acr{OTUs} table was normalized using edgeR (Version 3.28 or later) [@mccarthy2012].
...
It creates \acr{ASV} from the \acr{16S} sequencing data, without merging similar sequences at any threshold.
It is an alternative to the use of \acr{OTUs} which allows to compare results between studies and provides more resolution to identify differences on the fragment of \acr{16S} amplified.

We used SILVA v138.1 to annotate the \acr{16S} fragments whenever possible [@quast2013].
If we did not have access to the direct \acr{ASV} we used the annotation provided by the original authors of the dataset.

```{r workflow-process, fig.scap="Workflow of the analysis process.", fig.cap="Workflow of the main analysis process of the thesis. It shows the biopsy collection, extraction of genetic and transcriptomic material, sequencing and analysis. Created with BioRender.com"}
knitr::include_graphics("images/workflow_integration2.png")
```

## Integration methods

The main method used on this thesis has been regularized \acr{RGCCA} a method derived from the canonical correlation.
...
\acr{RGCCA} is implemented on the homonymous package [`RGCCA`](https://cran.r-project.org/package=RGCCA) [@tenenhaus2017] which was used here.
The method and implementation will be explained in detail in the next section.

```{r multiomic-datasets, fig.scap="Multi-omic relationships.", fig.cap="Multi-omic relationships on different datasets. Integration methods focus on relationships within datasets. Common relationships between datasets are used as confirmation/validation. Created with BioRender.com"}
...
It extends the \acr{PCA} to a two sources of data.
...
Over several years of progress on the field of canonical correlations [@tenenhaus_component-based_2008; @esposito_vinzi_bridge_2010; @tenenhaus_regularized_2011; @tenenhaus_regularized_2014; @tenenhaus_variable_2014; @tenenhaus_kernel_2015; @gloaguen2020], \acr{RGCCA} emerged with a generalization from canonical correlations extending the procedure to more than two sources of data [@R-RGCCA] and being made more flexible generalizing from other proposed methods.

#### Description

\acr{RGCCA} works with numeric matrices that can be as big as needed, as it is designed for datasets with more variables than samples ( $p \gg n$).
...
The method to perform the dimensional reduction using the sparse method consists on maximizing the same equation but with a different constraint: Specifically, \acr{RGCCA} with all $\tau_j = 1$, combined with an L1-penalty that gives rise to SGCCA:
...
The values of $s_j$ were estimated using the Schäfer method [@schäfer2005] when the block included \acr{16S} data or RNAseq, otherwise 1 was used.
...
\acr{RGCCA}, as other dimensional reduction techniques, provides specific weights for each variable on each dimension and a sample score on each dimension, together with quality scores.
...
To measure the quality of the model, the implementation provides indicators based on the \acr{AVE}.
\acr{RGCCA} returns an \acr{AVE} score for each block, which measures how the variables of the block correlate with the dimension component of the block.
It also provides two \acr{AVE} scores of the whole model: the inner, which measures how each dimension accounts for the variance, and the outer, which measures how variables correlate with the dimension components.
The closer the inner \acr{AVE} is to 1, the better the model adjusts to the data.
...
The following table summarizes the equivalent parameters needed on \acr{RGCCA} to work:
...
: (#tab:RGCCA-methods) Equivalences of RGCCA to other methods

There are other methods that can be performed with \acr{RGCCA}, for example: The classical canonical correlation analysis would be equivalent to data variance equal to 1 and shrinkage of 0.
Partial least squares (PLS) regression, which maximizes covariance, would be equal to data variance of 1 and shrinkage of 1 in \acr{RGCCA}
...
To help find the fitting model for the data we created an R package, named [inteRmodel](https://llrs.github.io/inteRmodel/), which helps finding the right model for the dataset via a bootstrapping procedure.
...
This is a method to examine covariant gene expression patterns between two blocks [@meng2014].
It is implemented on the package [omicade4](https://bioconductor.org/packages/omicade4).
On its core \acr{MCIA} maximizes the following formula:
...
\acr{MCIA} was used as a baseline method to compare the \acr{RGCCA} integration.
...
It has been widely used since its original publication, also in \acr{IBD} [@protiva2016].
...
For testing GSEA we used [`fgsea`](https://bioconductor.org/packages/fgsea) [@korotkevich2021] implementation for its speed and integration with other methods used in this thesis.
...
The \acr{PERMANOVA} method [@anderson2001; @warton2012], provided by the [vegan](https://cran.r-project.org/package=vegan) package on the `adonis` function, was used to test if microbiome data variance is due to other variables when using distances metrics.
...
We tested which variables, (sex, age, location, time since diagnosis, treatment) are important on the datasets with [globaltest](https://bioconductor.org/packages/globaltest) (Version 5.40 or later).
...
The bootstraps of the models where used to calculate the probability of a variable to be selected by \acr{RGCCA}
...
To compare between different models the \acr{AUC} of the \acr{ROC} was calculated with the [pROC](https://cran.r-project.org/package=pROC) package [@robin2011].
...
The \acr{ROC} curve is that where the true positive rate (TPR) or sensitivity, recall or hit rate is represented against the false positive rate (FPR) on the abscissa.
