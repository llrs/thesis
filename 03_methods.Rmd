...
| Location (corticoid, dipg, midl) | 20/22/10 |

: Table with the glioma's dataset characteristics
...
| Characteristics        | CD/UC |
|------------------------|-------|
| Controls/Disease       | 33/26 |
| Sex                    | 42/17 |
| Location (ileum/colon) | 30/29 |

: Table with CD/UC characteristics
...
This dataset included identifiers for the patients, whether the sample was from the pre-pouch ileum or from the pouch, the sex, the outcome of the procedure and an inflammatory severity score ISCORE.
...
Pathways of genes from the REACTOME database were tested on the weight of different models.

[GSVA](https://bioconductor.org/packages/GSVA) is a method that summarize the variables' numerical value so that they can be later used by other methods [@hänzelmann2013].
...
The $\alpha$-diversity index used can be calculated as follow:$Shannon = -\sum_{i=1}^N p_i \ln p_i$ and $Simpson = \dfrac{1}{\sum_{i=1}^N p_i^2}$ .

The effective Shannon and Simpson diversity index is the inverse of these formulas.
...
It is available via the [omicade](https://bioconductor.org/packages/omicade4) package on Bioconductor.
...
On the thesis it was used to avoid batch effect on the sequencing of the BARCELONA dataset.

<!--# TODO: Cite experDesign!! -->

Regularized generalized canonical correlation analysis, [RGCCA](https://cran.r-project.org/package=RGCCA), has been the main method used [@tenenhaus2017a].
It will be explained in detail on the next section.

To estimate if the selected features (genes or microorganism) have some biological meaningful contribution they are used to calculate if they can classify some features such as which gastrointestinal segment are each sample from or which type of disease does each patient have.
To calculate that the [pROC](https://cran.r-project.org/package=pROC) package was used [@robin2011].
The area under the curve of specificity and sensitivity measurements was compared between several values accounting for different genes and microorganisms.
...
It needs to have a complete case with no missing values (at the time of writing this thesis there is a new development version that replaces any missing value by a 0) and time is not considered as a specially variable.
...
To perform the dimensional reduction it follows these procedure:
...
The smaller the $s_j$, the larger the degree of sparsity for $a_j$, usually $s_j$ is calculated using Schäfer's method approach \[@schäfer2005\] unless they include metadata for which 1 is then used.
...
To help find the fitting model for the data I created an R package, [inteRmodel](https://llrs.github.io/inteRmodel/), which helps finding the right model and how fit it is for your data via a bootstrapping procedure.
