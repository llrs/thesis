...
<!--# Restart counter to start again the position -->
...
read_table <- function(dataset, model, option = 1) {
  ...
  if (option == 1) {
    m$Genes <- rownames(m)
    colnames(m) <- c("Weight", "Genes")
    m <- m[, c("Genes", "Weight")]
  } else if (option == 2) {
    m$Bacteria <- rownames(m)
    colnames(m) <- c("Weight", "Bacterias ID")
    m <- m[, c("Bacterias ID", "Weight")]
  }
  rownames(m) <- NULL
